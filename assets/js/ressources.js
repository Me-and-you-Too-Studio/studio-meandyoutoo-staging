(function(){
  ...
  async function load(){loading.hidden=false;loading.setAttribute('aria-busy','true');grid.hidden=true;empty.hidden=true;errorBox.hidden=true;count.textContent='';try{var results=await Promise.all([StudioAPI.request('/api/resources?limit=50'),StudioAPI.request('/api/resources/client')]);var publicItems=Array.isArray(results[0]&&results[0].items)?results[0].items:[];var privateItems=Array.isArray(results[1]&&results[1].items)?results[1].items:[];publicItems.forEach(function(item){item.source='blog';});privateItems.forEach(function(item){item.source='diverstory';});state.items=publicItems.concat(privateItems).sort(function(a,b){return new Date(b.publishedAt||0)-new Date(a.publishedAt||0);});fillCategories();loading.hidden=true;loading.setAttribute('aria-busy','false');render();}catch(err){console.error('Chargement ressources',err);loading.hidden=true;loading.setAttribute('aria-busy','false');errorBox.hidden=false;}}
