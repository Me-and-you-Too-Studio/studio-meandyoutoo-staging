(function(){
  ...
  function chooseCatalogVariant(variants){
    return new Promise(function(resolve){
      ...
      dialog.innerHTML='<form method="dialog" class="studio-modal-shell" style="max-width:680px;grid-template-columns:1fr"><div class="studio-modal-copy" style="padding-right:0"><p class="eyebrow">Version du diagnostic</p><h2>Choisissez le périmètre et la langue</h2><p class="studio-modal-message">Cette sélection détermine la version du catalogue utilisée pour votre campagne.</p></div><div class="studio-modal-field"><label>'+esc(scopeFieldLabel)+'</label><select data-variant-scope class="input" style="width:100%">'+scopes.map(function(x,i){return '<option value="'+i+'">'+esc(x.label)+'</option>';}).join('')+'</select></div><div class="studio-modal-field" style="margin-top:16px"><label>Langue disponible</label><select data-variant-locale class="input" style="width:100%"></select></div><div class="studio-modal-actions"><button type="button" class="button button-secondary" data-cancel>Annuler</button><button type="button" class="button button-primary" data-confirm>Continuer vers la composition</button></div></form>';
