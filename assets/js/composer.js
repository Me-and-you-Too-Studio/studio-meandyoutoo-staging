(() => {
  ...
  const state = { chapters: [], active: requestedChapter, project: null, library: [], libraryMode: 'add', replaceId: '', translationContexts:new Map() };
  ...
  function situationHtml(s,index){
    ...
    return `<article class="composer-situation ${tone} ${locked?'is-locked':''} ${customized?'has-customization':''}" data-situation-card="${esc(s.id)}">
      ...
      <div class="composer-translation-row"><button class="button button-ghost composer-translation-button" type="button" data-translations="${esc(s.id)}" hidden>🌐 Voir / modifier les traductions</button></div>
  ...
  function bindSituations(){
    ...
    document.querySelectorAll('[data-translations]').forEach(async b=>{const id=b.dataset.translations;try{const ctx=await getTranslationContext(id);if((ctx.locales||[]).filter(x=>x!=='fr').length){b.hidden=false;b.onclick=()=>openTranslationModal(id);}}catch(_){b.hidden=true;}});
  }
  ...
  function ensureTranslationStyles(){if(document.getElementById('composer-translation-styles'))return;const st=document.createElement('style');st.id='composer-translation-styles';st.textContent=`.composer-translation-row{margin:14px 0 2px}.translation-overlay{position:fixed;inset:0;background:rgba(12,35,55,.58);z-index:10000;display:grid;place-items:center;padding:24px}.translation-modal{background:#fff;border-radius:24px;width:min(1120px,96vw);max-height:92vh;overflow:auto;box-shadow:0 24px 70px rgba(0,0,0,.25)}.translation-head{padding:24px 28px 18px;border-bottom:1px solid #dce7ed;display:flex;justify-content:space-between;gap:20px}.translation-head h2{margin:4px 0 0}.translation-close{border:0;background:#eef5f7;border-radius:12px;width:42px;height:42px;font-size:24px}.translation-toolbar{padding:18px 28px;background:#f7fafb;display:flex;align-items:center;gap:12px}.translation-toolbar select{min-width:230px;height:46px;border:1px solid #bfd2dc;border-radius:12px;padding:0 14px;background:#fff}.translation-grid{display:grid;grid-template-columns:1fr 1fr;gap:20px;padding:24px 28px}.translation-pane{border:1px solid #dce7ed;border-radius:18px;padding:18px;background:#fff}.translation-pane.reference{background:#f7fafb}.translation-pane h3{margin:0 0 14px}.translation-pane textarea{width:100%;box-sizing:border-box;border:1px solid #bfd2dc;border-radius:12px;padding:12px 14px;font:inherit;line-height:1.5;resize:vertical}.translation-reference-text{white-space:pre-wrap;line-height:1.55;padding:12px 0}.translation-answer{padding:12px 0;border-top:1px solid #e5edf1}.translation-status{display:inline-flex;margin-left:8px;padding:4px 9px;border-radius:999px;background:#edf7f7;font-size:12px}.translation-foot{padding:18px 28px 24px;border-top:1px solid #dce7ed;display:flex;justify-content:flex-end;gap:12px}@media(max-width:760px){.translation-grid{grid-template-columns:1fr}.translation-modal{max-height:96vh}.translation-toolbar{align-items:flex-start;flex-direction:column}}`;document.head.appendChild(st);}
  async function openTranslationModal(id){const ctx=await getTranslationContext(id,true),targets=(ctx.locales||[]).filter(x=>x!=='fr');if(!targets.length)return;ensureTranslationStyles();const overlay=document.createElement('div');overlay.className='translation-overlay';overlay.innerHTML=`<section class="translation-modal" role="dialog" aria-modal="true"><header class="translation-head"><div><small>TRADUCTIONS DE LA SITUATION</small><h2>Comparer avec la référence française</h2></div><button class="translation-close" type="button" aria-label="Fermer">×</button></header><div class="translation-toolbar"><strong>Langue à vérifier</strong><select data-translation-locale>${targets.map(l=>`<option value="${esc(l)}">${esc(localeLabel(l))}</option>`).join('')}</select><span data-translation-status></span></div><div class="translation-grid"><section class="translation-pane reference" data-fr-pane></section><section class="translation-pane" data-target-pane></section></div><footer class="translation-foot"><button class="button button-ghost" type="button" data-cancel>Annuler</button><button class="button button-primary" type="button" data-save-translation>Enregistrer et fermer</button></footer></section>`;document.body.appendChild(overlay);const close=()=>overlay.remove();overlay.querySelector('.translation-close').onclick=close;overlay.querySelector('[data-cancel]').onclick=close;const select=overlay.querySelector('[data-translation-locale]');const fr=overlay.querySelector('[data-fr-pane]'),target=overlay.querySelector('[data-target-pane]');
    function draw(){const loc=select.value,t=ctx.translations?.[loc]||{answers:[]};fr.innerHTML=`<h3>Français (FR) · référence</h3><div class="translation-reference-text">${esc(ctx.french.content)}</div><h4>Réponses</h4>${(ctx.french.answers||[]).map((a,i)=>`<div class="translation-answer"><strong>Réponse ${i+1}</strong><div class="translation-reference-text">${esc(a.content)}</div></div>`).join('')}`;target.innerHTML=`<h3>${esc(localeLabel(loc))}<span class="translation-status">${t.status==='missing'?'À compléter':t.status==='client_version'?'Personnalisée':'Traduction Me&YouToo'}</span></h3><textarea rows="4" data-target-content placeholder="Traduction à compléter">${esc(t.content||'')}</textarea><h4>Réponses</h4>${(ctx.french.answers||[]).map((a,i)=>{const ta=(t.answers||[]).find(x=>String(x.id)===String(a.id))||{};return `<div class="translation-answer"><strong>Réponse ${i+1}</strong><textarea rows="2" data-target-answer="${esc(a.id)}" placeholder="Traduction à compléter">${esc(ta.content||'')}</textarea></div>`}).join('')}`;}
    select.onchange=draw;draw();overlay.querySelector('[data-save-translation]').onclick=async()=>{const loc=select.value,content=target.querySelector('[data-target-content]').value.trim(),answers=[...target.querySelectorAll('[data-target-answer]')].map(x=>({id:Number(x.dataset.targetAnswer),content:x.value.trim()}));try{await api(`/api/projects/${projectId}/situations/${id}/translations/${encodeURIComponent(loc)}`,{method:'PATCH',body:JSON.stringify({content,answers})});state.translationContexts.delete(String(id));close();showMessage(`${localeLabel(loc)} enregistrée.`,'success');}catch(e){showMessage(e.message);}};
  }
