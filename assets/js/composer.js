(() => {
  ...
  async function removeSituation(id){const ch=state.chapters[state.active];if(!confirm('Supprimer cette situation du chapitre ?'))return;try{await api(`/api/projects/${projectId}/situations/${id}`,{method:'DELETE'});ch.situations=ch.situations.filter(s=>String(s.id)!==String(id));render();showMessage('La situation a été supprimée du brouillon.','success');}catch(e){showMessage(e.message);}}

  function currentCatalogIds(){const ch=state.chapters[state.active];return new Set(ch.situations.flatMap(s=>[String(s.catalog_situation_id||''),String(s.source_id||''),canonical(s.content)]).filter(Boolean));}
