...
(function(){
  ...
  function setupNotifications(attempt){
    ...
    function notificationUrl(item){var url=String(item.action_url||''),meta=item.metadata||{},type=String(item.type||'').toLowerCase(),title=String(item.title||'').toLowerCase();var projectId=meta.projectId||meta.project_id||'',organizationId=meta.organizationId||meta.organization_id||item.organization_id||'';if((type.includes('submission')||title.includes('configuration'))&&projectId)return 'client.html?'+(organizationId?'organizationId='+encodeURIComponent(organizationId)+'&':'')+'projectId='+encodeURIComponent(projectId);if((type.includes('communication')||type.includes('kit'))&&projectId)return 'kit-communication.html?projectId='+encodeURIComponent(projectId);if(type.includes('pack'))return 'admin.html?tab=clients&filter=pack';if(IS_ADMIN&&INTERFACE_MODE==='admin'&&/^admin\.html\?projectId=/i.test(url))url=url.replace(/^admin\.html/i,'client.html');return url;}
