(function(){
  ...
  async function load(){try{const data=await StudioAPI.request('/api/catalog/themes');const live=new Map((data.themes||[]).map(t=>[norm(t.slug),t]));themes=plannedThemes.map(base=>mergeTheme(base,live.get(norm(base.slug))));for(const t of data.themes||[])if(!themes.some(x=>norm(x.slug)===norm(t.slug)))themes.push(mergeTheme({slug:norm(t.slug)},t));fillFilters();render()}catch(e){themes=plannedThemes.slice();fillFilters();render()}}
