(function(){
  ...
  const plannedThemes=[
    ...
    {slug:'handicap',title:'Handicap',description:fallbackDescriptions.handicap,situation_count:46,chapter_count:4,library_situation_count:0,country_codes:['FR'],available_locales:['fr']},
  ...
  function render(){
    ...
    root.innerHTML=filtered.map(t=>{const loc=locales(t),sc=scopes(t),co=countries(t),cat=Number(t.catalog_situation_count??t.situation_count??0),lib=Number(t.library_situation_count||0);return `<article class="card topic-card"><div class="topic-illustration"><img src="assets/img/illustrations/theme-${esc(illustrationSlug(t.slug))}.png" alt="" onerror="this.closest('.topic-illustration').style.display='none'"></div><div class="topic-body"><h3>${esc(t.title)}</h3><p>${esc(t.description||fallbackDescriptions[t.slug]||'Une thématique issue du référentiel propriétaire Me&YouToo.')}</p><div class="topic-capabilities"><div><strong>Périmètres</strong><div class="topic-capability-values">${compact([...sc.map(scopeLabel),...co.map(countryLabel)],x=>x,2)}</div></div><div><strong>Langues</strong><div class="topic-capability-values">${compact(loc,localeLabel,2)}</div></div></div><div class="topic-content-split"><div class="topic-content-count is-catalog"><span>Catalogue</span><strong>${cat}</strong><small>situations de référence</small></div><div class="topic-content-count is-library"><span>Bibliothèque</span><strong>${lib}</strong><small>situations complémentaires</small></div></div><div class="meta-row"><span class="meta">${Number(t.chapter_count||0)} chapitres</span></div><a class="button button-secondary" href="${href(t)}">Découvrir la thématique</a></div></article>`}).join('')||'<article class="card topic-card library-no-result"><div class="topic-body"><h3>Aucune thématique disponible</h3><p>Aucune thématique ne correspond à ces critères. Modifiez le périmètre ou la langue.</p></div></article>';
  }
