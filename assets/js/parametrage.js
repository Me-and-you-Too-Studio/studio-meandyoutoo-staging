(()=>{
  ...
  function openResourceModal(index=null){
    ...
    const syncType=()=>{const type=dialog.querySelector('input[name="result-resource-type"]:checked')?.value||'link';linkField.hidden=type!=='link';documentField.hidden=type!=='document';if(type==='document'){if(item.type==='legacy_document'&&item.legacyFilename)help.textContent=`Document historique attendu : ${item.legacyFilename}. Sélectionnez le PDF correspondant.`;else if(existingDoc)help.textContent=`PDF actuel : ${existingDoc.filename}. Choisissez un nouveau fichier uniquement si vous souhaitez le remplacer pour cette ressource.`;else help.textContent='Sélectionnez le PDF à proposer aux répondants.';}error.hidden=true;};
  ...
  function applyReadOnlyUI(){
    ...
    const readOnlyHidden='[data-example],#add-socio,[data-socio-remove],[data-opt-remove],[data-opt-add],[data-sub-add],[data-sub-remove],[data-sub-opt-add],[data-sub-opt-remove]'+(currentUser.role==='admin'?'':',#add-result-resource');
  ...
  $('settings-form').addEventListener('submit',async event=>{
    ...
    if(!respondentTitle)return show('Le titre visible est obligatoire.');if(!project?.review_mode&&launchDate<iso(5))return show('La date de lancement doit être au minimum à J+5.');if(closeDate<=launchDate)return show('La date de clôture doit être postérieure à la date de lancement.');if(!socio.length)return show('Au moins une donnée d’analyse doit être présente.');if(invalidSocio())return show('Chaque donnée et chaque sous-critère doivent avoir un intitulé et au moins deux réponses renseignées.');const incompleteResource=resultResources.find(r=>!resourceComplete(r));if(incompleteResource)return show(incompleteResource.type==='legacy_document'?`Le PDF historique « ${incompleteResource.legacyFilename} » doit être rattaché avant de pouvoir être utilisé.`:'Chaque ressource doit avoir un texte et soit une URL valide, soit un fichier PDF.');const resultButtons=resultResources.filter(resourceComplete);
