(()=>{
  ...
  const api=(url,opt={})=>window.StudioAPI.request(url,opt);let baseTitle='',socio=[],quota=null;
  ...
  const GENDER_Q='Quel est votre genre ?',AGE_Q='Quelle est votre tranche d’âge ?';
  const GENDER_OPTIONS=['Homme','Femme','Non binaire','Autre'],AGE_OPTIONS=['Moins de 30 ans','30 à 44 ans','45 à 54 ans','55 ans et plus'];
  const criterionExamples=[
    {q:'À quelle Business Unit appartenez-vous ?',opts:['Business Unit 1','Business Unit 2']},
    {q:'Dans quelle région travaillez-vous ?',opts:['Région 1','Région 2']},
    {q:'Quelle est votre fonction ?',opts:['Fonction 1','Fonction 2']},
    {q:'Êtes-vous manager ou collaborateur·rice ?',opts:['Manager','Collaborateur·rice']}
  ];
  const defaultSocio=()=>theme==='sexisme'?[
    {kind:'gender',q:GENDER_Q,opts:GENDER_OPTIONS.map(label=>({label,n:0}))},
    {kind:'age',q:AGE_Q,opts:AGE_OPTIONS.map(label=>({label,n:0}))}
  ]:[{q:'Nouvelle donnée',opts:[{label:'Réponse 1',n:0},{label:'Réponse 2',n:0}]}];
  ...
  const sameText=(a,b)=>String(a||'').trim().toLocaleLowerCase('fr')===String(b||'').trim().toLocaleLowerCase('fr');
  function standardizeSexismeSocio(items){
    if(theme!=='sexisme')return items;
    const source=Array.isArray(items)?items:[];
    const oldGender=source.find(c=>c.kind==='gender'||/genre/i.test(c.q||''));
    const oldAge=source.find(c=>c.kind==='age'||/tranche.*âge|tranche.*age|âge|age/i.test(c.q||''));
    const countFor=(criterion,label)=>{
      const aliases=label==='Autre'?['Autre','Autre / Je ne souhaite pas répondre']:label==='Non binaire'?['Non binaire','Non-binaire']: [label];
      const found=criterion?.opts?.find(o=>aliases.some(a=>sameText(o.label,a)));
      return Number(found?.n)||0;
    };
    const gender={kind:'gender',q:GENDER_Q,opts:GENDER_OPTIONS.map(label=>({label,n:countFor(oldGender,label)}))};
    const age=oldAge?{kind:'age',q:AGE_Q,opts:AGE_OPTIONS.map(label=>({label,n:countFor(oldAge,label)}))}:null;
    const others=source.filter(c=>c!==oldGender&&c!==oldAge);
    return [gender,...(age?[age]:[]),...others];
  }
  ...
  function renderSocio(){
    $('socio-list').innerHTML=socio.map((criterion,i)=>{
      const locked=theme==='sexisme'&&(criterion.kind==='gender'||criterion.kind==='age');
      const mandatory=theme==='sexisme'&&criterion.kind==='gender';
      const benchmark=locked?`<span class="socio-benchmark-badge">Référentiel benchmark · libellés verrouillés</span>`:'';
      const rows=criterion.opts.map((option,j)=>{
        const protectedGender=criterion.kind==='gender'&&(option.label==='Homme'||option.label==='Femme');
        const removable=criterion.kind==='gender'?!protectedGender:!locked&&criterion.opts.length>2;
        if(locked){
          const state=optionState(option.n);
          return `<div class="socio-option-unit"><div class="socio-option"><div class="socio-answer-field locked"><span>Réponse</span><input value="${esc(option.label)}" readonly aria-label="Réponse standardisée"></div><div class="socio-count-field ${state.cls}"><span>Effectif estimé</span><input data-opt-n="${i}:${j}" type="number" min="0" placeholder="Ex. 25" value="${Number(option.n)||''}" aria-label="Effectif estimé"></div>${criterion.kind==='gender'&&!protectedGender?`<button type="button" class="socio-option-remove" data-opt-remove="${i}:${j}" aria-label="Supprimer ${esc(option.label)}">×</button>`:'<span class="socio-option-lock" title="Réponse obligatoire">🔒</span>'}</div><div class="socio-option-hint ${state.cls}">${state.hint}</div></div>`;
        }
        return `<div class="socio-option-block">${answerRow(option,`${i}:${j}`,removable)}${option.subcriterion?`<div class="socio-subcriterion"><div class="socio-sub-head"><span class="socio-level-badge">Niveau 2 · si « ${esc(option.label||'cette réponse')} »</span><button type="button" class="socio-sub-remove" data-sub-remove="${i}:${j}">Retirer</button></div><div class="field"><label>Question complémentaire</label><input data-sub-q="${i}:${j}" value="${esc(option.subcriterion.q)}"></div><div class="socio-sub-options">${option.subcriterion.opts.map((child,k)=>answerRow(child,`${i}:${j}:${k}`,option.subcriterion.opts.length>2,true)).join('')}</div><button class="button button-ghost" type="button" data-sub-opt-add="${i}:${j}">+ Ajouter une sous-réponse</button></div>`:`<button class="socio-add-sub" type="button" data-sub-add="${i}:${j}">+ Ajouter un sous-critère pour cette réponse</button>`}</div>`;
      }).join('');
      return `<article class="socio-card ${locked?'socio-card-standard':''}"><div class="socio-card-head"><span class="socio-number"><small>Critère</small>${i+1}</span><div class="field socio-question"><label>Question posée aux répondants ${benchmark}</label><input data-socio-q="${i}" value="${esc(criterion.q)}" ${locked?'readonly':''}></div><button class="button button-danger-soft" type="button" data-socio-remove="${i}" ${mandatory||socio.length<=1?'disabled':''}>${mandatory?'Obligatoire':'Supprimer'}</button></div><div class="socio-options">${rows}</div>${locked?'':`<button class="button button-ghost" type="button" data-opt-add="${i}">+ Ajouter une réponse</button>`}</article>`;
    }).join('');
  ...
  const nextDay=date=>{if(!date)return'';const d=new Date(date+'T12:00:00');d.setDate(d.getDate()+1);return d.toISOString().slice(0,10);};
  function syncCloseDate(){
    const launch=$('launch-date').value,minClose=nextDay(launch);
    $('close-date').min=minClose;
    if($('close-date').value&&minClose&&$('close-date').value<minClose)$('close-date').value='';
  }
  async function load(){try{if(!projectId){const project=await window.StudioProject.createOrResume(theme);location.replace(`parametrage.html?theme=${encodeURIComponent(theme)}&projectId=${encodeURIComponent(project.id)}`);return;}const data=await api(`/api/projects/${projectId}/composer`),meta=data.project||{};await loadQuota();baseTitle=meta.base_title||meta.theme_title||meta.title||'Compréhension du sexisme';$('param-theme').textContent=meta.theme_title||'Compréhension du sexisme';$('campaign-name').value=meta.campaign_name||baseTitle;$('respondent-title').value=meta.respondent_title||meta.respondent_title_default||baseTitle;$('intro').value=stripHtml(meta.introduction_html||meta.theme_introduction_html||'');$('launch-date').min=iso(5);$('launch-date').value=meta.launch_date?String(meta.launch_date).slice(0,10):iso(5);$('close-date').value=meta.close_date?String(meta.close_date).slice(0,10):'';syncCloseDate();$('nb-respondents').value=meta.estimated_respondents||'';socio=standardizeSexismeSocio(normalizedSocio(Array.isArray(meta.sociodemo)&&meta.sociodemo.length?meta.sociodemo:defaultSocio()));renderSocio();renderQuota();$('param-back').href=`personnalisation.html?theme=${encodeURIComponent(theme)}&projectId=${encodeURIComponent(projectId)}`;await api(`/api/projects/${projectId}/progress`,{method:'PATCH',body:JSON.stringify({currentStep:'parametrage'})});}catch(error){show(error.message);}}
  ...
  $('add-socio').onclick=()=>{$('socio-examples')?.toggleAttribute('hidden');};
  document.querySelectorAll('[data-socio-example]').forEach(el=>el.onclick=()=>{
    const key=el.dataset.socioExample;
    if(key==='age'&&theme==='sexisme'){
      if(!socio.some(c=>c.kind==='age'))socio.push({kind:'age',q:AGE_Q,opts:AGE_OPTIONS.map(label=>({label,n:0}))});
    }else{
      const ex=criterionExamples[Number(key)];
      if(ex)socio.push({q:ex.q,opts:ex.opts.map(label=>({label,n:0}))});
    }
    $('socio-examples').hidden=true;renderSocio();
  });
  $('launch-date').onchange=syncCloseDate;
  $('nb-respondents').oninput=()=>{updateVigilance();renderQuota();};$('close-packs').onclick=()=>{const panel=$('inline-packs');panel.hidden=true;const toggle=$('toggle-packs');if(toggle)toggle.textContent='Voir les packs disponibles';};window.addEventListener('studio:pack-requested',loadQuota);bindPackToggle();
  $('settings-form').addEventListener('submit',async event=>{event.preventDefault();const campaignName=$('campaign-name').value.trim(),respondentTitle=$('respondent-title').value.trim(),introductionHtml=$('intro').value.trim(),launchDate=$('launch-date').value,closeDate=$('close-date').value,nbRespondents=Number($('nb-respondents').value)||0;if(!respondentTitle||!introductionHtml)return show('Le titre visible et l’introduction sont obligatoires.');if(launchDate<iso(5))return show('La date de lancement doit être au minimum à J+5.');if(closeDate&&closeDate<=launchDate)return show('La date de clôture doit être postérieure à la date de lancement.');if(!nbRespondents||!socio.length)return show('Indiquez le nombre de répondants et au moins une donnée d’analyse.');if(invalidSocio())return show('Chaque donnée et chaque sous-critère doivent avoir un intitulé et au moins deux réponses renseignées.');try{await api(`/api/projects/${projectId}/settings`,{method:'PATCH',body:JSON.stringify({campaignName,respondentTitle,introductionHtml,launchDate,closeDate,nbRespondents,sociodemo:socio})});location.href=`validation.html?theme=${encodeURIComponent(theme)}&projectId=${encodeURIComponent(projectId)}`;}catch(error){show(error.message);}});load();
})();
