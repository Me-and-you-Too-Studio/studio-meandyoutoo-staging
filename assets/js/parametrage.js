(()=>{
  ...
  function bindSocio(){
    ...
    document.querySelectorAll('[data-tree-n]').forEach(el=>el.oninput=()=>{const option=getOptionByPath(el.dataset.treeN);if(option)option.n=Number(el.value)||0;updateOptionVisual(el);});
    document.querySelectorAll('[data-socio-remove]').forEach(el=>el.onclick=()=>{socio.splice(+el.dataset.socioRemove,1);renderSocio();});
    document.querySelectorAll('[data-tree-option-remove]').forEach(el=>el.onclick=()=>{const [criterionPath,index]=el.dataset.treeOptionRemove.split('|'),criterion=getCriterionByPath(criterionPath);criterion?.opts?.splice(Number(index),1);renderSocio();});
    document.querySelectorAll('[data-tree-option-add]').forEach(el=>el.onclick=()=>{const criterion=getCriterionByPath(el.dataset.treeOptionAdd);criterion?.opts?.push({label:el.dataset.treeOptionAdd.includes('.')?'Nouvelle sous-réponse':'Nouvelle réponse',n:0,subcriteria:[]});renderSocio();});
    document.querySelectorAll('[data-tree-child-add]').forEach(el=>el.onclick=()=>{const option=getOptionByPath(el.dataset.treeChildAdd);if(!option)return;option.subcriteria=Array.isArray(option.subcriteria)?option.subcriteria:[];option.subcriteria.push(newSubcriterion());renderSocio();});
    document.querySelectorAll('[data-tree-criterion-remove]').forEach(el=>el.onclick=()=>{const info=criterionParentInfo(el.dataset.treeCriterionRemove);if(!info)return;const parent=getCriterionByPath(info.parentPath);parent?.opts?.[info.optionIndex]?.subcriteria?.splice(info.childIndex,1);renderSocio();});
  }
