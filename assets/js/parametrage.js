(()=>{
  ...
  let baseTitle='';let socio=[];let quota=null;
  ...
  function renderQuota(){
    const root=$('param-credit-card');if(!root)return;
    if(!quota){root.innerHTML='<div><h3>Pack non renseigné</h3><p>Le volume sera confirmé par Me&YouToo. Vous pouvez déjà consulter les packs disponibles.</p></div><a class="button" href="packs.html">Voir les packs et commander</a>';return;}
    const need=Number($('nb-respondents').value)||0,remaining=Number(quota.passations_remaining)||0;
    const unlimited=String(quota.passations_quota)==='Infinity';
    let status='Renseignez le nombre de répondants estimé.',tone='';
    if(need){if(unlimited||remaining>=need){status='✓ Solde suffisant pour cette campagne.';}else{status='⚠ Il manque '+fmt(need-remaining)+' passation(s) pour couvrir le besoin estimé.';tone=' warn';}}
    root.className='passation-credit-card'+tone;
    root.innerHTML=`<div><h3>${esc(quota.name||'Votre entreprise')}</h3><p>${quota.pack_expires_at?'Pack valable jusqu’au '+new Date(quota.pack_expires_at).toLocaleDateString('fr-FR'):'Date de validité non renseignée'}</p><div class="credit-stats"><div class="credit-stat"><strong>${fmt(quota.passations_quota)}</strong><span>Achetées</span></div><div class="credit-stat"><strong>${fmt(quota.passations_used)}</strong><span>Utilisées</span></div><div class="credit-stat"><strong>${fmt(remaining)}</strong><span>Restantes</span></div><div class="credit-stat"><strong>${need||'—'}</strong><span>Prévues ici</span></div></div><div class="credit-status">${status}</div></div><a class="button" href="packs.html">Voir les packs et commander</a>`;
  }
  async function loadQuota(){
    try{const d=await api('/api/me/organization-quota');quota=d.organization||null;}catch(e){quota=null;}renderQuota();
  }
  ...
  async function load(){
    try{
      ...
      const d=await api(`/api/projects/${projectId}/composer`),meta=d.project||{};await loadQuota();baseTitle=meta.base_title||meta.theme_title||meta.title||'Compréhension du sexisme';
  ...
  $('add-socio').onclick=()=>{socio.push({q:'Nouvelle donnée',opts:[{label:'Réponse 1',n:0},{label:'Réponse 2',n:0}]});renderSocio();};$('nb-respondents').oninput=()=>{updateVigilance();renderQuota();};
