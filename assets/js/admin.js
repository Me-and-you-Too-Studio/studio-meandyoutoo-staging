...
const normalizedStatus=p=>p.status==='configuration_submitted'?'review_pending':p.status==='completed'?'unpublished':p.status;
  ...
  function migrationDisplayTranslation(payload,locale){
    ...
    if(l==='fr'&&(!direct||migrationIsTechnicalCountryLabel(direct))){
      const english=migrationTranslationText(payload,'en');
      if(english&&!migrationIsTechnicalCountryLabel(english))return{text:english,fallback:true,sourceLocale:'en'};
    }
    return{text:direct,fallback:false,sourceLocale:l};
  }
  function migrationCountryLocaleMap(metaPayload={}){
    const countries=Array.isArray(metaPayload.countries)?metaPayload.countries:[],localeRows=Array.isArray(metaPayload.locales)?metaPayload.locales:[];
    ...
    countries.forEach(row=>{
      const code=String(row.country_code||row.code||'').trim().toUpperCase();
      if(!code)return;
  ...
  function migrationVisibleLocales(m,context={}){
    ...
    const codes=[...new Set([...(p.countryCodes||[]),...(p.country_codes||[])].map(x=>String(x||'').trim().toUpperCase()).filter(Boolean))];
  ...
  const libraryCountryLabel=code=>({FR:'France',BE:'Belgique',CH:'Suisse',CA:'Canada',LU:'Luxembourg',ES:'Espagne'}[String(code||'').toUpperCase()]||String(code||''));
  function libraryThemeScopeBadges(theme){
    const locales=(Array.isArray(theme?.available_locales)?theme.available_locales:['fr'])
      .map(x=>String(x||'').toUpperCase()).filter(Boolean);
    const scopeRaw=String(theme?.cultural_legal_scope||'').trim().toUpperCase();
    const scopeLabel=scopeRaw?libraryCountryLabel(scopeRaw):'Universel';
    return `<div class="admin-library-theme-scope">
      <span class="admin-library-theme-scope-tag is-scope"><b>Périmètre culturel et légal</b> ${esc(scopeLabel)}</span>
      <span class="admin-library-theme-scope-tag is-language"><b>Langues disponibles</b> ${locales.join(' · ')}</span>
    </div>`;
  }
