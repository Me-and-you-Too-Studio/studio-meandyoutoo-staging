(function(){
  ...
  const projectCountryNamesByNumericCode={'032':'Argentine','040':'Autriche','056':'Belgique','072':'Botswana','076':'Brésil','100':'Bulgarie','120':'Cameroun','124':'Canada','140':'République centrafricaine','152':'Chili','156':'Chine','158':'Taïwan','178':'Congo','191':'Croatie','203':'Tchéquie','208':'Danemark','250':'France','276':'Allemagne','288':'Ghana','300':'Grèce','324':'Guinée','344':'Hong Kong','356':'Inde','380':'Italie','384':'Côte d’Ivoire','392':'Japon','400':'Jordanie','410':'Corée du Sud','422':'Liban','430':'Liberia','442':'Luxembourg','450':'Madagascar','466':'Mali','480':'Maurice','484':'Mexique','498':'Moldavie','504':'Maroc','528':'Pays-Bas','566':'Nigeria','578':'Norvège','591':'Panama','616':'Pologne','620':'Portugal','624':'Guinée-Bissau','642':'Roumanie','643':'Fédération de Russie','686':'Sénégal','694':'Sierra Leone','703':'Slovaquie','724':'Espagne','752':'Suède','756':'Suisse','788':'Tunisie','818':'Égypte','826':'Royaume-Uni','834':'Tanzanie','840':'États-Unis','854':'Burkina Faso','858':'Uruguay'};
  const normalizeLocale=v=>String(v||'').trim().toLowerCase().replaceAll('_','-');
  const normalizeProjectCountry=v=>String(v||'').trim().toUpperCase();
  ...
  function filteredOrgs(){const q=$('#org-search').value.toLowerCase().trim(),sector=$('#org-sector-filter').value,theme=$('#org-theme-filter')?.value||'',languageFilter=$('#org-language-filter')?.value||'',scopeFilter=$('#org-scope-filter')?.value||'',sort=$('#org-sort').value;const rows=state.organizations.filter(o=>{const text=[o.name,orgCountryLabel(o.country_code),...orgSectors(o),...orgUsers(o).flatMap(u=>[u.first_name,u.last_name,u.email,u.job_title]),accountManagerName(o.account_manager_user_id),...orgProjects(o).flatMap(p=>[p.campaign_name,p.theme_title,...projectLocales(p),...projectLocales(p).map(localeLabel)])].join(' ').toLowerCase();const folderOk=state.clientFolderFilter==='all'||(state.clientFolderFilter==='favorites'?o.admin_favorite===true:(state.clientFolderFilter==='unclassified'?!o.admin_client_folder_id:String(o.admin_client_folder_id||'')===state.clientFolderFilter));const languageOk=languageFilter!=='multilingual'||orgMultilingualProjects(o).length>0;const scopeOk=!scopeFilter||orgProjects(o).some(p=>{const countries=projectCountries(p);return scopeFilter==='__multi__'?countries.length>1:countries.includes(scopeFilter);});return folderOk&&languageOk&&scopeOk&&(!q||text.includes(q))&&(!sector||orgSectors(o).includes(sector))&&(!theme||orgThemes(o).some(([name])=>name===theme))&&matches(o);});rows.sort((a,b)=>{const x=stats(a),y=stats(b);if(sort==='recent')return y.last-x.last;if(sort==='remaining')return(x.rem??Infinity)-(y.rem??Infinity);if(sort==='usage')return y.rate-x.rate;if(sort==='campaigns')return y.ps.length-x.ps.length;if(sort==='name')return String(a.name||'').localeCompare(String(b.name||''),'fr',{sensitivity:'base'});const score=s=>s.toPublish*50+(s.pending?60:0)+(s.low?35:0)+(s.live?30:0)+(s.underused?15:0);return score(y)-score(x);});return rows;}
...
Les tags existants seront conservés. La bibliothèque complémentaire Sexisme est incluse. Les autres bibliothèques complémentaires seront intégrées plus tard à partir de vos fichiers Excel ou Word.`;const confirmed=await StudioModal.confirm({eyebrow:'Catalogue Me&YouToo',title:'Intégrer les tags proposés ?',message,type:'info',cancelLabel:'Annuler',confirmLabel:'Intégrer les tags'});if(!confirmed){autoTag.disabled=false;autoTag.textContent='Analyser et intégrer les tags';return;}autoTag.textContent='Intégration…';const result=await StudioAPI.request('/api/admin/catalog/tagging-apply',{method:'POST',body:'{}'});await refreshLibrary();await StudioModal.alert({eyebrow:'Catalogue Me&YouToo',title:'Tags intégrés',message:`${result.updated||0} situation${Number(result.updated||0)>1?'s':''} enrichie${Number(result.updated||0)>1?'s':''}, base et bibliothèque complémentaire Sexisme incluses.`,type:'success',confirmLabel:'Fermer'});}catch(error){showError(error.message);}finally{const b=$('[data-auto-tag-catalog]');if(b){b.disabled=false;b.textContent='Analyser et intégrer les tags';}}};
  ...
  ['org-search','org-sector-filter','org-theme-filter','org-language-filter','org-scope-filter','org-sort'].forEach(id=>$('#'+id).addEventListener(id==='org-search'?'input':'change',()=>{renderChips();renderOrganizations();}));
  if($('#org-jump'))$('#org-jump').addEventListener('change',e=>{const id=e.target.value;if(id)location.href='client.html?organizationId='+encodeURIComponent(id);});['campaign-search','campaign-status'].forEach(id=>$('#'+id).addEventListener(id==='campaign-search'?'input':'change',renderCampaigns));['account-search','account-status'].forEach(id=>$('#'+id).addEventListener(id==='account-search'?'input':'change',renderAccounts));$('#activity-search').oninput=renderActivity;
