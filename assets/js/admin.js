...
const normalizedStatus=p=>p.status==='configuration_submitted'?'review_pending':p.status==='completed'?'unpublished':p.status;
  ...
  function renderOrganizations(){
    ...
    function adminCampaignIsDraft(p){return normalizedStatus(p)==='draft';}
    ...
    function renderCampaigns(){
      ...
      $('#admin-campaigns').innerHTML=rows.map(p=>{const countries=projectCountries(p),locales=projectLocales(p),lifecycle=adminCampaignLifecycle(p),raw=normalizedStatus(p),stateBadges=[];
        // Les badges sont cumulatifs : publication et état temporel ne s'excluent pas.
        if(raw==='draft')stateBadges.push('<span class="badge badge-status">Brouillon</span>');
        if(raw==='unpublished')stateBadges.push('<span class="badge badge-status">Dépubliée</span>');
        if(raw==='archived')stateBadges.push('<span class="badge badge-status">Archivée</span>');
        if(adminCampaignReviewStatus(raw))stateBadges.push('<span class="badge badge-status">À traiter</span>');
        if(lifecycle==='scheduled')stateBadges.push('<span class="badge badge-status">Programmée</span>');
        if(lifecycle==='ongoing')stateBadges.push('<span class="badge badge-status">En cours</span>');
        if(adminCampaignIsFinished(p))stateBadges.push('<span class="badge badge-status">Terminée</span>');
        if(!stateBadges.length)stateBadges.push(`<span class="badge badge-status">${esc(labels[raw]||raw||'—')}</span>`);
        const signals=[adminCampaignEndingSoon(p)?'<span class="admin-campaign-signal is-ending">🔴 Fin proche</span>':'',adminCampaignHasResults(p)?'<span class="admin-campaign-signal is-results">📊 Résultats disponibles</span>':''].filter(Boolean).join(''),meta=[...countries.slice(0,4).map(c=>`<span>${esc(projectCountryLabel(c))}</span>`),...locales.slice(0,5).map(l=>`<span>${esc(localeLabel(l))}</span>`)].join('');return `<article class="admin-campaign-card" data-project-card="${p.id}"><div class="admin-campaign-card-main"><div class="admin-campaign-card-badges">${stateBadges.join('')}${signals}</div><h3>${esc(p.campaign_name||p.title||'Sans nom')}</h3><p><strong>${esc(p.organizationName)}</strong> · ${esc(p.theme_title||'Thématique non renseignée')}</p>${meta?`<div class="admin-campaign-meta-tags">${meta}</div>`:''}</div><dl><div><dt>Étape</dt><dd>${esc(p.current_step||'—')}</dd></div><div><dt>Dernière modification</dt><dd>${dateTime(p.updated_at)}</dd></div><div><dt>Dates de diffusion</dt><dd>${date(p.launch_date)} → ${date(p.close_date)}</dd></div></dl><a class="button button-secondary" href="client.html?projectId=${p.id}">Ouvrir dans le dossier client</a>${adminCommentPanel('project',p.id)}</article>`;}).join('')||'<div class="card admin-empty">Aucune campagne ne correspond à ces filtres.</div>';
