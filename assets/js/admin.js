...
const normalizedStatus=p=>p.status==='configuration_submitted'?'review_pending':p.status==='completed'?'unpublished':p.status;
  ...
  function libraryThemeScopeBadges(theme){
    const locales=(Array.isArray(theme?.available_locales)?theme.available_locales:['fr']).map(x=>String(x||'').toUpperCase()).filter(Boolean);
    ...
    return `<div class="admin-library-theme-scope">${tags.map(tag=>`<span class="admin-library-theme-scope-tag is-scope"><b>Tag</b> ${esc(tag)}</span>`).join('')}${countries.length?`<span class="admin-library-theme-scope-tag is-scope"><b>Pays couverts</b> ${countries.map(c=>esc(libraryCountryLabel(c))).join(' · ')}</span>`:''}<span class="admin-library-theme-scope-tag is-language"><b>Langues disponibles</b> ${locales.join(' · ')}</span></div>`;
  }
  ...
  function libraryLocalizationDetails(entity,entityType){
    ...
    if(!generic.length&&!variants.length)return `<p class="admin-library-localization-empty">Aucune traduction enregistrée pour ce contenu.</p>`;
  ...
  const themeForm=$('#library-theme-form');if(themeForm)themeForm.onsubmit=async e=>{e.preventDefault();if(!themeForm.reportValidity())return;const id=$('#library-theme-id').value,payload={title:$('#library-theme-title').value.trim(),description:$('#library-theme-description').value.trim(),baseTitle:$('#library-theme-base-title').value.trim(),respondentTitle:$('#library-theme-respondent-title').value.trim(),introductionHtml:$('#library-theme-introduction').value.trim(),resultTitle:$('#library-theme-result-title').value.trim(),resultSentence:$('#library-theme-result-sentence').value.trim(),active:$('#library-theme-active').checked,adminTags:$('#library-theme-admin-tags').value.split(',').map(x=>x.trim()).filter(Boolean)};try{await StudioAPI.request(id?'/api/admin/catalog/themes/'+id:'/api/admin/catalog/themes',{method:id?'PATCH':'POST',body:JSON.stringify(payload)});$('#library-theme-dialog').close();await refreshLibrary();}catch(error){showError(error.message);}};
