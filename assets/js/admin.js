...
const normalizedStatus=p=>p.status==='configuration_submitted'?'review_pending':p.status==='completed'?'unpublished':p.status;
  ...
  function globalPickerHtml(targetChapter){
    ...
    return `<section class="admin-global-catalog-picker" data-global-picker="${targetChapter.id}"><div class="admin-global-picker-head"><div><strong>Catalogue global Me&amp;YouToo</strong><p>Ces situations existent déjà dans d’autres autodiagnostics. Vous les réutilisez ici sans les dupliquer.</p></div><button type="button" class="button button-ghost button-small" data-close-global-picker>Fermer</button></div><div class="admin-global-picker-help"><span class="admin-info-dot" tabindex="0" title="Base du thème : la situation sera proposée par défaut dans cet autodiagnostic. Bibliothèque complémentaire : elle sera disponible comme alternative uniquement dans cet autodiagnostic. Le client n’a jamais accès au catalogue global.">i</span><span>Choisissez la provenance, les langues à activer ici, puis la façon de réutiliser la situation.</span></div><div class="admin-global-picker-filters"><label><span>Thématique d’origine</span><select data-global-picker-theme><option value="">Toutes les thématiques</option>${sourceThemes.map(t=>`<option value="${t.id}" ${themeFilter===String(t.id)?'selected':''}>${esc(t.title)}</option>`).join('')}</select></label><label><span>Chapitre d’origine</span><select data-global-picker-source-chapter><option value="">Tous les chapitres</option>${sourceChapters.map(ch=>`<option value="${ch.id}" ${sourceChapterFilter===String(ch.id)?'selected':''}>${esc(ch.title)}</option>`).join('')}</select></label></div><label class="admin-global-picker-search"><span>Rechercher par mot-clé, contexte ou texte</span><input type="search" data-global-picker-search value="${esc(state.libraryGlobalPickerQuery||'')}" placeholder="Ex. harcèlement, réunion, chantier, industrie, management…"></label><div class="admin-global-picker-results">${rows.length?rows.map(row=>`<article class="admin-global-picker-card" data-global-situation-card="${row.situation.id}"><div><div class="admin-global-picker-source"><span>${esc(row.theme.title)}</span><b>›</b><span>${esc(row.chapter.title)}</span></div><p>${esc(libraryRealContent(row.situation))}</p>${situationTagHtml(row.situation)}${languageChoices(row)}</div><div class="admin-global-picker-actions"><button type="button" class="button button-secondary button-small" data-link-global-situation="${row.situation.id}" data-target-chapter="${targetChapter.id}" data-placement="library">Réutiliser comme situation complémentaire</button><button type="button" class="button button-primary button-small" data-link-global-situation="${row.situation.id}" data-target-chapter="${targetChapter.id}" data-placement="base">Réutiliser dans la base du thème</button></div></article>`).join(''):`<p class="admin-library-empty-line">Aucune situation ne correspond à ces filtres.</p>`}</div></section>`;
  }
