...
const normalizedStatus=p=>p.status==='configuration_submitted'?'review_pending':p.status==='completed'?'unpublished':p.status;
  ...
  function globalPickerHtml(targetChapter){
    ...
    const themeId=String(state.libraryGlobalPickerThemeId||'');
    const sourceChapterId=String(state.libraryGlobalPickerSourceChapterId||'');
    const already=new Set((targetChapter.situations||[]).map(si=>String(si.id)));
    const allRows=globalCatalogSituations().filter(row=>String(row.chapter.id)!==String(targetChapter.id)&&!already.has(String(row.situation.id)));
    const themes=[...new Map(allRows.map(row=>[String(row.theme.id),row.theme])).values()].sort((a,b)=>String(a.title||'').localeCompare(String(b.title||''),'fr'));
    const chapters=[...new Map(allRows.filter(row=>!themeId||String(row.theme.id)===themeId).map(row=>[String(row.chapter.id),row.chapter])).values()].sort((a,b)=>String(a.title||'').localeCompare(String(b.title||''),'fr'));
    const rows=allRows.filter(row=>{
      if(themeId&&String(row.theme.id)!==themeId)return false;
      if(sourceChapterId&&String(row.chapter.id)!==sourceChapterId)return false;
      if(!q)return true;
    ...
    const info=`<span class="admin-global-picker-info" tabindex="0" role="button" aria-label="Comprendre la réutilisation d’une situation"><span class="admin-global-picker-info-icon" aria-hidden="true">i</span><span class="admin-global-picker-info-popover"><strong>Cette situation existe déjà dans le Catalogue Me&amp;YouToo.</strong><span>Vous ne créez pas une copie : vous la <b>réutilisez dans cet autodiagnostic</b>. Sa thématique et son chapitre d’origine restent inchangés.</span><span><b>Base du thème</b> : elle sera proposée par défaut dans la composition initiale de cet autodiagnostic.</span><span><b>Bibliothèque complémentaire</b> : elle sera disponible comme alternative uniquement dans cet autodiagnostic. Le client ne voit jamais le catalogue global.</span></span></span>`;
    return `<section class="admin-global-catalog-picker" data-global-picker="${targetChapter.id}"><div class="admin-global-picker-head"><div><strong>Catalogue global Me&amp;YouToo</strong><p>Recherche transversale réservée aux admins. Filtrez par thématique et chapitre pour retrouver une situation existante.</p></div><button type="button" class="button button-ghost button-small" data-close-global-picker>Fermer</button></div><div class="admin-global-picker-toolbar"><label class="admin-global-picker-search"><span>Rechercher par mot-clé, contexte ou texte</span><input type="search" data-global-picker-search value="${esc(state.libraryGlobalPickerQuery||'')}" placeholder="Ex. harcèlement, réunion, chantier, industrie, management…"></label><label><span>Thématique d’origine</span><select data-global-picker-theme><option value="">Toutes les thématiques</option>${themes.map(t=>`<option value="${t.id}" ${String(t.id)===themeId?'selected':''}>${esc(t.title)}</option>`).join('')}</select></label><label><span>Chapitre d’origine</span><select data-global-picker-source-chapter><option value="">Tous les chapitres</option>${chapters.map(ch=>`<option value="${ch.id}" ${String(ch.id)===sourceChapterId?'selected':''}>${esc(ch.title)}</option>`).join('')}</select></label></div><div class="admin-global-picker-destination-help"><span><strong>Situation existante :</strong> choisissez comment la réutiliser dans cet autodiagnostic.</span>${info}</div><div class="admin-global-picker-results">${rows.length?rows.map(row=>`<article class="admin-global-picker-card"><div><div class="admin-global-picker-source"><span>${esc(row.theme.title)}</span><b>›</b><span>${esc(row.chapter.title)}</span></div><p>${esc(libraryRealContent(row.situation))}</p>${situationTagHtml(row.situation)}</div><div class="admin-global-picker-actions"><button type="button" class="button button-secondary button-small" data-link-global-situation="${row.situation.id}" data-target-chapter="${targetChapter.id}" data-placement="library">Réutiliser comme situation complémentaire</button><button type="button" class="button button-primary button-small" data-link-global-situation="${row.situation.id}" data-target-chapter="${targetChapter.id}" data-placement="base">Réutiliser dans la base du thème</button></div></article>`).join(''):`<p class="admin-library-empty-line">Aucune situation ne correspond à ces filtres.</p>`}</div></section>`;
  }
  ...
  function bindLibraryActions(){
    ...
    $$('[data-global-picker-theme]').forEach(select=>select.onchange=()=>{state.libraryGlobalPickerThemeId=select.value;state.libraryGlobalPickerSourceChapterId='';const chapterId=String(state.libraryGlobalPickerChapterId||'');state.libraryExpandedChapters.add(chapterId);renderLibraryAdmin();});
    $$('[data-global-picker-source-chapter]').forEach(select=>select.onchange=()=>{state.libraryGlobalPickerSourceChapterId=select.value;const chapterId=String(state.libraryGlobalPickerChapterId||'');state.libraryExpandedChapters.add(chapterId);renderLibraryAdmin();});
    $$('[data-link-global-situation]').forEach(b=>b.onclick=async()=>{try{b.disabled=true;await StudioAPI.request('/api/admin/catalog/chapters/'+b.dataset.targetChapter+'/situation-placements',{method:'POST',body:JSON.stringify({situationId:Number(b.dataset.linkGlobalSituation),placement:b.dataset.placement})});const chapterId=String(b.dataset.targetChapter);await refreshLibrary();state.libraryExpandedChapters.add(chapterId);state.libraryGlobalPickerChapterId=chapterId;renderLibraryAdmin();}catch(error){b.disabled=false;showError(error.message);}});
