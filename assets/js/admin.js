...
const normalizedStatus=p=>p.status==='configuration_submitted'?'review_pending':p.status==='completed'?'unpublished':p.status;
  ...
  function openNewOrganization(){ $('#org-form').reset();contactsRoot.innerHTML='';syncQuota();const manager=$('#org-account-manager');if(manager)manager.innerHTML=accountManagerOptions('');orgDialog.showModal(); }
  ...
  $('#create-org').onclick=async e=>{e.preventDefault();if(!$('#org-form').reportValidity())return;const selected=selectedSectors();if(!selected.length)return;try{await StudioAPI.request('/api/admin/organizations',{method:'POST',body:JSON.stringify({name:$('#org-name').value.trim(),sectors:selected,contacts:contactsPayload(),passationsQuota:selectedQuota(),packStartedAt:$('#org-pack-start').value||null,packExpiresAt:$('#org-expiry').value||null,accountManagerUserId:$('#org-account-manager')?.value||null})});orgDialog.close();await load();activateTab('clients');}catch(error){showError(error.message);}};
