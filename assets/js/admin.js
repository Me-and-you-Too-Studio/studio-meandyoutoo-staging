...
const normalizedStatus=p=>p.status==='configuration_submitted'?'review_pending':p.status==='completed'?'unpublished':p.status;
  ...
  async function openMediaUsageManager(videoId){
    ...
    const chapterRows=(state.catalogThemes||[]).flatMap(t=>(t.chapters||[]).map(ch=>({theme:t,chapter:ch}))).sort((a,b)=>String(a.theme.title||'').localeCompare(String(b.theme.title||''),'fr')||Number(a.chapter.position||0)-Number(b.chapter.position||0));
    ...
    const associationHtml=()=>associations.length?associations.map(a=>`<article class="admin-media-usage-row"><div><strong>${esc(a.theme_title||'Thématique')}</strong><span>${esc(a.chapter_title||'Chapitre')} · ${esc(placementLabel(a))}${a.locale?' · '+esc(String(a.locale).toUpperCase()):''}</span></div><button class="button button-danger-soft button-small" type="button" data-remove-catalog-media="${a.id}">Retirer</button></article>`).join(''):'<p class="admin-media-usage-empty">Aucune association catalogue. La vidéo n’est héritée automatiquement par aucune campagne.</p>';
    ...
    const directHtml=()=>direct.length?direct.map(a=>`<article class="admin-media-usage-row"><div><strong>${esc(a.campaign_name||a.project_title||'Campagne')}</strong><span>${esc(a.organization_name||'Client')} · ${esc(a.chapter_title||'Chapitre')} · ${esc(placementLabel(a))}${a.locale?' · '+esc(String(a.locale).toUpperCase()):''}</span><small>Association spécifique à cette campagne</small></div><button class="button button-danger-soft button-small" type="button" data-remove-project-media="${a.id}" data-project-id="${a.project_id}">Retirer</button></article>`).join(''):'<p class="admin-media-usage-empty">Aucune association spécifique à une campagne.</p>';
    dialog.innerHTML=`<form method="dialog" class="admin-media-usage-shell"><button class="admin-dialog-close" value="cancel" aria-label="Fermer">×</button><p class="eyebrow">Médiathèque · utilisations</p><h2>${esc(video?.title||usage.video?.title||'Vidéo')}</h2><p class="admin-media-delete-lead">Gérez la source centrale. Une association catalogue est automatiquement héritée par toutes les campagnes reliées au même chapitre, y compris les campagnes historiques importées.</p><div class="admin-media-usage-info"><strong>ⓘ Héritage automatique</strong><span>Pour remettre une vidéo sur plusieurs anciens autodiagnostics, associez-la ici au bon chapitre du catalogue : vous n’avez pas à rouvrir les campagnes une par une. Les associations spécifiques servent uniquement aux exceptions legacy.</span></div><section><div class="admin-media-usage-section-head"><div><h3>Associations catalogue</h3><p>Source de vérité pour les nouvelles campagnes et les campagnes historiques mappées.</p></div></div><div data-catalog-usage-list>${associationHtml()}</div><div class="admin-media-usage-add"><label><span>Thématique · chapitre</span><select data-usage-chapter><option value="">Choisir un chapitre…</option>${chapterRows.map(({theme,chapter})=>`<option value="${chapter.id}">${esc(theme.title)} — ${esc(chapter.title)}</option>`).join('')}</select></label><label><span>Emplacement</span><select data-usage-placement><option value="profile_result">Profils de restitution</option><option value="after_chapter">Après le chapitre</option></select></label><label><span>Profil</span><select data-usage-profile><option value="">Les 3 profils</option><option value="0">Profil 1</option><option value="1">Profil 2</option><option value="2">Profil 3</option></select></label><label><span>Langue <small>(facultatif)</small></span><input data-usage-locale maxlength="12" placeholder="ex. fr"></label><button class="button button-primary" type="button" data-add-catalog-usage>Associer au catalogue</button></div></section><section><div class="admin-media-usage-section-head"><div><h3>Campagnes qui héritent de cette vidéo</h3><p>Ce ne sont pas des associations ajoutées campagne par campagne.</p></div><span class="admin-media-usage-count">${inherited.length}</span></div><div class="admin-media-usage-scroll" data-inherited-usage-list>${inheritedHtml()}</div></section><section><div class="admin-media-usage-section-head"><div><h3>Exceptions / associations spécifiques</h3><p>Utilisées pour un chapitre historique qui ne doit pas dépendre du catalogue.</p></div><span class="admin-media-usage-count">${direct.length}</span></div><div data-direct-usage-list>${directHtml()}</div></section><div class="admin-media-delete-actions"><button class="button button-secondary" value="cancel">Fermer</button></div></form>`;
    document.body.appendChild(dialog);dialog.showModal();
    const refresh=async()=>{dialog.close();await openMediaUsageManager(videoId);};
    const placement=dialog.querySelector('[data-usage-placement]'),profile=dialog.querySelector('[data-usage-profile]');
    const syncProfile=()=>{profile.disabled=placement.value!=='profile_result';if(profile.disabled)profile.value='';};placement.onchange=syncProfile;syncProfile();
    dialog.querySelector('[data-add-catalog-usage]').onclick=async()=>{const chapterId=dialog.querySelector('[data-usage-chapter]').value;if(!chapterId){showError('Choisissez d’abord un chapitre.');return;}const button=dialog.querySelector('[data-add-catalog-usage]');button.disabled=true;button.textContent='Association…';try{await StudioAPI.request('/api/admin/catalog/chapters/'+encodeURIComponent(chapterId)+'/media',{method:'POST',body:JSON.stringify({videoId:Number(videoId),placement:placement.value,profilePosition:profile.value===''?null:Number(profile.value),locale:dialog.querySelector('[data-usage-locale]').value.trim()||null,active:true})});state.catalogLoaded=false;await refresh();}catch(error){button.disabled=false;button.textContent='Associer au catalogue';showError(error.message);}};
