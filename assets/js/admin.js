(function(){
  ...
  function renderClientJump(){const select=$('#org-jump');if(!select)return;const current=select.value;const rows=[...state.organizations].sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''),'fr',{sensitivity:'base'}));select.innerHTML='<option value="">Choisir un client…</option>'+rows.map(o=>`<option value="${esc(o.id)}">${esc(o.name||'Client sans nom')}</option>`).join('');if(rows.some(o=>String(o.id)===String(current)))select.value=current;}
...
const normalizedStatus=p=>p.status==='configuration_submitted'?'review_pending':p.status==='completed'?'unpublished':p.status;
  ...
  function renderOrganizations(){
    ...
    function adminCampaignRows(){return state.organizations.flatMap(o=>orgProjects(o).map(p=>({...p,organizationName:o.name,organizationId:o.id})));}
    ...
    function adminCampaignOngoingStatus(status){return['published','active'].includes(status);}
    function adminCampaignFinishedStatus(status){return['completed','closed'].includes(status);}
    function adminCampaignEndingSoon(p){if(!['scheduled','published','active'].includes(p.status)||!p.close_date)return false;const now=new Date(),limit=new Date(now.getTime()+14*24*60*60*1000),d=new Date(p.close_date);return!Number.isNaN(d.getTime())&&d>=now&&d<=limit;}
    function adminCampaignHasResults(p){return Boolean(String(p.communication_results_url||'').trim());}
    function adminCampaignQuickMatch(p,key){if(!key||key==='all')return true;if(key==='endingSoon')return adminCampaignEndingSoon(p);if(key==='results')return adminCampaignHasResults(p);if(key==='review')return adminCampaignReviewStatus(p.status);if(key==='ongoing')return adminCampaignOngoingStatus(p.status);if(key==='finished')return adminCampaignFinishedStatus(p.status);return p.status===key;}
    function adminCampaignQuickLabel(key){return({all:'Toutes',endingSoon:'Fin proche',results:'Résultats',draft:'Brouillon',review:'À traiter',scheduled:'Programmée',ongoing:'En cours',finished:'Terminée',unpublished:'Dépubliée',archived:'Archivée'})[key]||labels[key]||key;}
    ...
    function adminCampaignPeriodMatch(p,value){if(!value||value==='all')return true;const now=new Date();if(value==='current'){if(!p.launch_date)return false;const start=new Date(p.launch_date),end=p.close_date?new Date(p.close_date):null;return!Number.isNaN(start.getTime())&&start<=now&&(!end||Number.isNaN(end.getTime())||end>=now)&&adminCampaignOngoingStatus(p.status);}if(value==='upcoming'){if(!p.launch_date)return false;const start=new Date(p.launch_date);return!Number.isNaN(start.getTime())&&start>now;}if(value.startsWith('year:')){const year=Number(value.split(':')[1]);if(!year)return true;const start=p.launch_date?new Date(p.launch_date):(p.created_at?new Date(p.created_at):null),end=p.close_date?new Date(p.close_date):null,yearStart=new Date(year,0,1),yearEnd=new Date(year,11,31,23,59,59,999);return Boolean(start&&!Number.isNaN(start.getTime())&&start<=yearEnd&&(!end||Number.isNaN(end.getTime())||end>=yearStart));}return true;}
    ...
    function renderAdminCampaignQuickFilters(rows){
      ...
      root.innerHTML=keys.filter(key=>key==='all'||count(key)>0).map(key=>`<button type="button" class="admin-campaign-filter-chip ${state.campaignQuickFilter===key?'is-active':''}" data-admin-campaign-filter="${esc(key)}"><span>${adminCampaignQuickIcon(key)} ${esc(adminCampaignQuickLabel(key))}</span><strong>${fmt(count(key))}</strong></button>`).join('');
    ...
    function renderAdminCampaignKpis(rows){
      const counts={all:rows.length,draft:rows.filter(p=>p.status==='draft').length,review:rows.filter(p=>adminCampaignReviewStatus(p.status)).length,ongoing:rows.filter(p=>adminCampaignOngoingStatus(p.status)).length,finished:rows.filter(p=>adminCampaignFinishedStatus(p.status)).length};
    ...
    function renderCampaigns(){
      ...
      const rows=allRows.filter(p=>{
        ...
        return adminCampaignQuickMatch(p,state.campaignQuickFilter)&&(!status||p.status===status)&&(!theme||p.theme_title===theme)&&adminCampaignPeriodMatch(p,period)&&(!scope||countries.includes(scope))&&(!locale||locales.includes(locale))&&(!client||String(p.organizationId)===String(client))&&(!q||haystack.includes(q));
      });
      ...
      $('#admin-campaigns').innerHTML=rows.map(p=>{const countries=projectCountries(p),locales=projectLocales(p),signals=[adminCampaignEndingSoon(p)?'<span class="admin-campaign-signal is-ending">🔴 Fin proche</span>':'',adminCampaignHasResults(p)?'<span class="admin-campaign-signal is-results">📊 Résultats disponibles</span>':''].filter(Boolean).join(''),meta=[...countries.slice(0,4).map(c=>`<span>${esc(projectCountryLabel(c))}</span>`),...locales.slice(0,5).map(l=>`<span>${esc(localeLabel(l))}</span>`)].join('');return `<article class="admin-campaign-card" data-project-card="${p.id}"><div class="admin-campaign-card-main"><div class="admin-campaign-card-badges"><span class="badge badge-status">${labels[p.status]||esc(p.status)}</span>${signals}</div><h3>${esc(p.campaign_name||p.title||'Sans nom')}</h3><p><strong>${esc(p.organizationName)}</strong> · ${esc(p.theme_title||'Thématique non renseignée')}</p>${meta?`<div class="admin-campaign-meta-tags">${meta}</div>`:''}</div><dl><div><dt>Étape</dt><dd>${esc(p.current_step||'—')}</dd></div><div><dt>Dernière modification</dt><dd>${dateTime(p.updated_at)}</dd></div><div><dt>Dates</dt><dd>${date(p.launch_date)} → ${date(p.close_date)}</dd></div></dl><a class="button button-secondary" href="client.html?projectId=${p.id}">Ouvrir dans le dossier client</a>${adminCommentPanel('project',p.id)}</article>`;}).join('')||'<div class="card admin-empty">Aucune campagne ne correspond à ces filtres.</div>';
      bindAdminComments($('#admin-campaigns'));
    }
  ...
  function renderMediaLibrary(){
    ...
    $$('[data-delete-media-library]').forEach(b=>b.onclick=async()=>{const v=state.mediaLibrary.find(x=>String(x.id)===String(b.dataset.deleteMediaLibrary));if(!await StudioModal.confirm({eyebrow:'Médiathèque',title:'Supprimer cette vidéo ?',message:`« ${v?.title||'Cette vidéo'} » sera supprimée définitivement de la médiathèque.`,type:'danger',cancelLabel:'Conserver',confirmLabel:'Supprimer'}))return;try{await StudioAPI.request('/api/admin/media-library/'+b.dataset.deleteMediaLibrary,{method:'DELETE'});await loadMediaLibrary();state.catalogLoaded=false;}catch(error){showError(error.message);}});
  }
...
Les tags existants seront conservés. La bibliothèque complémentaire Sexisme est incluse. Les autres bibliothèques complémentaires seront intégrées plus tard à partir de vos fichiers Excel ou Word.`;const confirmed=await StudioModal.confirm({eyebrow:'Catalogue Me&YouToo',title:'Intégrer les tags proposés ?',message,type:'info',cancelLabel:'Annuler',confirmLabel:'Intégrer les tags'});if(!confirmed){autoTag.disabled=false;autoTag.textContent='Analyser et intégrer les tags';return;}autoTag.textContent='Intégration…';const result=await StudioAPI.request('/api/admin/catalog/tagging-apply',{method:'POST',body:'{}'});await refreshLibrary();await StudioModal.alert({eyebrow:'Catalogue Me&YouToo',title:'Tags intégrés',message:`${result.updated||0} situation${Number(result.updated||0)>1?'s':''} enrichie${Number(result.updated||0)>1?'s':''}, base et bibliothèque complémentaire Sexisme incluses.`,type:'success',confirmLabel:'Fermer'});}catch(error){showError(error.message);}finally{const b=$('[data-auto-tag-catalog]');if(b){b.disabled=false;b.textContent='Analyser et intégrer les tags';}}};
  ...
  if($('#org-jump'))$('#org-jump').addEventListener('change',e=>{const id=e.target.value;if(id)location.href='client.html?organizationId='+encodeURIComponent(id);});['campaign-search','campaign-status','campaign-theme','campaign-period','campaign-scope','campaign-locale','campaign-client','campaign-sort'].forEach(id=>{const el=$('#'+id);if(el)el.addEventListener(id==='campaign-search'?'input':'change',renderCampaigns);});$$('[data-admin-campaign-kpi]').forEach(button=>button.addEventListener('click',()=>{state.campaignQuickFilter=button.dataset.adminCampaignKpi||'all';renderCampaigns();}));['account-search','account-status'].forEach(id=>$('#'+id).addEventListener(id==='account-search'?'input':'change',renderAccounts));$('#activity-search').oninput=renderActivity;
