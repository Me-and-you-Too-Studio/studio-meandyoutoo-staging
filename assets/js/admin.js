(function(){
  ...
  function renderAll(){renderKpis();renderOverview();renderSectors();renderThemeFilter();renderLocaleFilter();renderLanguageCountFilter();renderScopeFilter();renderClientJump();renderChips();renderClientFolderBar();renderFolderSelect();renderOrganizations();renderCampaigns();renderAccounts();renderAdministrators();renderActivity();applyRequestedView();}
  ...
  function selectedDiagnosticFilters(){return{theme:$('#org-theme-filter')?.value||'',locale:$('#org-locale-filter')?.value||'',languageCounts:selectedLanguageCounts(),scope:$('#org-scope-filter')?.value||''};}
  function projectCountryKeys(p){return [...new Set(projectCountries(p).map(code=>String(projectCountryLabel(code)||'').trim().toLocaleLowerCase('fr')).filter(Boolean))];}
  function projectIsFranceOnly(p){const keys=projectCountryKeys(p);return keys.length===1&&keys[0]==='france';}
  function projectMatchesDiagnosticFilters(p,filters=selectedDiagnosticFilters()){
    if(filters.theme&&String(p.theme_title||'').trim()!==filters.theme)return false;
    const locales=projectLocales(p);
    if(filters.locale&&!locales.includes(normalizeLocale(filters.locale)))return false;
    const languageCounts=Array.isArray(filters.languageCounts)?filters.languageCounts:[];
    if(languageCounts.length&&!languageCounts.includes(locales.length))return false;
    if(filters.scope){
      const countries=projectCountries(p),countryKeys=projectCountryKeys(p);
      if(filters.scope==='__france__'){if(!projectIsFranceOnly(p))return false;}
      else if(filters.scope==='__international__'){if(!countries.length||projectIsFranceOnly(p))return false;}
      else if(filters.scope==='__multi__'){if(countryKeys.length<=1)return false;}
      else{const wanted=String(projectCountryLabel(filters.scope)||'').trim().toLocaleLowerCase('fr');if(!countryKeys.includes(wanted))return false;}
    }
    return true;
  }
  function orgMatchesDiagnosticFilters(o,filters=selectedDiagnosticFilters()){if(!filters.theme&&!filters.locale&&!(filters.languageCounts||[]).length&&!filters.scope)return true;return orgProjects(o).some(p=>projectMatchesDiagnosticFilters(p,filters));}
  ...
  function renderLocaleFilter(){
    const select=$('#org-locale-filter');if(!select)return;
    const current=normalizeLocale(select.value),theme=$('#org-theme-filter')?.value||'',languageCounts=selectedLanguageCounts(),scope=$('#org-scope-filter')?.value||'';
    const projects=state.organizations.flatMap(orgProjects).filter(p=>projectMatchesDiagnosticFilters(p,{theme,locale:'',languageCounts,scope}));
    const byLabel=new Map();projects.flatMap(projectLocales).forEach(code=>{const normalized=normalizeLocale(code),label=localeLabel(normalized);if(!byLabel.has(label))byLabel.set(label,normalized);});
    const entries=[...byLabel.entries()].sort((a,b)=>a[0].localeCompare(b[0],'fr',{sensitivity:'base'}));
    select.innerHTML='<option value="">Toutes les langues disponibles</option>'+entries.map(([label,code])=>`<option value="${esc(code)}">${esc(label)}</option>`).join('');
    const match=entries.find(([,code])=>code===current);select.value=match?match[1]:'';
  }
  function renderLanguageCountFilter(){const root=$('#org-language-count-options'),summary=$('[data-language-count-summary]');if(!root||!summary)return;const selected=new Set(selectedLanguageCounts());const theme=$('#org-theme-filter')?.value||'',locale=$('#org-locale-filter')?.value||'',scope=$('#org-scope-filter')?.value||'';const projects=state.organizations.flatMap(orgProjects).filter(p=>projectMatchesDiagnosticFilters(p,{theme,locale,languageCounts:[],scope}));const counts=new Map();projects.forEach(p=>{const n=projectLocales(p).length;if(n>0)counts.set(n,(counts.get(n)||0)+1);});const values=[...counts.keys()].sort((a,b)=>a-b);root.innerHTML=values.length?values.map(n=>{const count=counts.get(n)||0,label=`${n} langue${n>1?'s':''}`;return `<label class="admin-multicheck-simple ${selected.has(n)?'is-selected':''}"><input type="checkbox" data-language-count value="${n}" ${selected.has(n)?'checked':''}><span>${label}</span><small>${count} AD</small></label>`;}).join(''):'<span class="admin-multicheck-empty">Aucun AD avec langue renseignée</span>';const active=values.filter(n=>selected.has(n));summary.textContent=!active.length?'Nombre de langues':active.length<=2?active.map(n=>`${n} langue${n>1?'s':''}`).join(' · '):`${active.length} sélections`;}
  function renderScopeFilter(){
    const select=$('#org-scope-filter');if(!select)return;
    const current=select.value,theme=$('#org-theme-filter')?.value||'',locale=$('#org-locale-filter')?.value||'',languageCounts=selectedLanguageCounts();
    const projects=state.organizations.flatMap(orgProjects).filter(p=>projectMatchesDiagnosticFilters(p,{theme,locale,languageCounts,scope:''}));
    const byLabel=new Map();projects.flatMap(projectCountries).forEach(code=>{const label=projectCountryLabel(code);if(!byLabel.has(label))byLabel.set(label,code);});
    const values=[...byLabel.entries()].sort((a,b)=>a[0].localeCompare(b[0],'fr',{sensitivity:'base'}));
    select.innerHTML='<option value="">Tous les périmètres</option><optgroup label="Accès rapide"><option value="__france__">France</option><option value="__international__">International</option></optgroup><optgroup label="Pays">'+values.map(([label,code])=>`<option value="${esc(code)}">${esc(label)}</option>`).join('')+'</optgroup>';
    if(['__france__','__international__'].includes(current)||values.some(([,code])=>code===current))select.value=current;else select.value='';
  }
...
const normalizedStatus=p=>p.status==='configuration_submitted'?'review_pending':p.status==='completed'?'unpublished':p.status;
  ...
  function renderOrganizations(){
    ...
    function populateAdminCampaignFilters(rows){
      ...
      const rawScopes=[...new Set(rows.flatMap(projectCountries))].sort((a,b)=>projectCountryLabel(a).localeCompare(projectCountryLabel(b),'fr',{sensitivity:'base'}));
      // Le legacy peut contenir plusieurs codes pour un même pays (ex. DE et 276).
      // Le filtre travaille donc sur le libellé canonique, pas sur le code brut.
      const scopeByLabel=new Map();
      rawScopes.forEach(code=>{const label=projectCountryLabel(code),key=String(label||'').trim().toLocaleLowerCase('fr');if(label&&!scopeByLabel.has(key))scopeByLabel.set(key,{key,label});});
      const detailedScopes=[...scopeByLabel.values()].filter(item=>item.label!=='France'&&item.label!=='International');
      const quickScopeOptions='<optgroup label="Accès rapide"><option value="__france__">France</option><option value="__international__">International</option></optgroup>';
      const detailedScopeOptions=detailedScopes.length?'<optgroup label="Pays">'+detailedScopes.map(item=>`<option value="country:${esc(item.key)}">${esc(item.label)}</option>`).join('')+'</optgroup>':'';
      preserve(scope,'<option value="">Tous les périmètres</option>'+quickScopeOptions+detailedScopeOptions);
      const rawLocales=[...new Set(rows.flatMap(projectLocales))].sort((a,b)=>localeLabel(a).localeCompare(localeLabel(b),'fr',{sensitivity:'base'}));
      const localeByLabel=new Map();
      rawLocales.forEach(code=>{const label=localeLabel(code),key=String(label||'').trim().toLocaleLowerCase('fr');if(label&&!localeByLabel.has(key))localeByLabel.set(key,{key,label});});
      const locales=[...localeByLabel.values()];
      preserve(locale,'<option value="">Toutes les langues</option>'+locales.map(item=>`<option value="language:${esc(item.key)}">${esc(item.label)}</option>`).join(''));
    ...
    function renderCampaigns(){
      ...
      const rows=allRows.filter(p=>{
        const countries=projectCountries(p),locales=projectLocales(p),countryKeys=countries.map(c=>String(projectCountryLabel(c)||'').trim().toLocaleLowerCase('fr')),localeKeys=locales.map(l=>String(localeLabel(l)||'').trim().toLocaleLowerCase('fr')),haystack=[p.campaign_name,p.title,p.theme_title,p.organizationName,labels[p.status]||p.status,...countries.map(projectCountryLabel),...locales.map(localeLabel)].join(' ').toLowerCase();
        const lifecycle=adminCampaignLifecycle(p),statusMatch=!status||adminCampaignQuickMatch(p,status);
        const franceOnly=countryKeys.length===1&&countryKeys[0]==='france';
        const scopeKey=scope.startsWith('country:')?scope.slice(8):'';
        const localeKey=locale.startsWith('language:')?locale.slice(9):'';
        const scopeMatch=!scope||(scope==='__france__'?franceOnly:scope==='__international__'?(countries.length>0&&!franceOnly):scopeKey?countryKeys.includes(scopeKey):false);
        const localeMatch=!locale||(localeKey?localeKeys.includes(localeKey):locales.includes(locale));
        return adminCampaignQuickMatch(p,state.campaignQuickFilter)&&statusMatch&&(!theme||p.theme_title===theme)&&adminCampaignPeriodMatch(p,period)&&scopeMatch&&localeMatch&&(!client||String(p.organizationId)===String(client))&&(!q||haystack.includes(q));
      });
      ...
      root.querySelectorAll('[data-toggle-campaign-meta]').forEach(button=>button.onclick=()=>{const tags=button.closest('.admin-campaign-meta-tags');if(!tags)return;const kind=button.dataset.toggleCampaignMeta,expanded=button.getAttribute('aria-expanded')==='true';tags.querySelectorAll(`[data-overflow-kind="${kind}"]`).forEach(tag=>tag.hidden=expanded);button.setAttribute('aria-expanded',expanded?'false':'true');button.textContent=expanded?(button.dataset.collapsedLabel||'+'):'Réduire';});
  ...
  async function openMediaUsageManager(videoId){
    const video=state.mediaLibrary.find(v=>String(v.id)===String(videoId));
    let usage;
    try{
      if(!(state.catalogThemes||[]).length){
        const catalog=await StudioAPI.request('/api/admin/catalog/themes');
        state.catalogThemes=catalog.themes||[];
        state.catalogLoaded=true;
      }
      usage=await StudioAPI.request('/api/admin/media-library/'+encodeURIComponent(videoId)+'/usages');
    }catch(error){showError(error.message);return;}
    document.querySelector('#admin-media-usage-dialog')?.remove();
    const dialog=document.createElement('dialog');dialog.id='admin-media-usage-dialog';dialog.className='admin-dialog admin-media-usage-dialog';
    const associations=Array.isArray(usage.associations)?usage.associations:[];
    const inherited=Array.isArray(usage.inheritedCampaigns)?usage.inheritedCampaigns:(Array.isArray(usage.campaigns)?usage.campaigns:[]);
    const excluded=Array.isArray(usage.excludedCampaigns)?usage.excludedCampaigns:[];
    const direct=Array.isArray(usage.directAssignments)?usage.directAssignments:[];
    const themes=(state.catalogThemes||[]).slice().sort((a,b)=>String(a.title||'').localeCompare(String(b.title||''),'fr',{sensitivity:'base'}));
    const localeLabels={fr:'Français (FR)',en:'Anglais (EN)',bg:'Bulgare (BG)',br:'Portugais Brésil (BR)',de:'Allemand (DE)',es:'Espagnol (ES)',it:'Italien (IT)',ja:'Japonais (JA)','ko-kr':'Coréen (KO-KR)',nl:'Néerlandais (NL)','nl-be':'Néerlandais Belgique (NL-BE)',pl:'Polonais (PL)',pt:'Portugais (PT)',ro:'Roumain (RO)',ru:'Russe (RU)','sv-se':'Suédois (SV-SE)',tr:'Turc (TR)',zf:'Chinois simplifié (ZF)',zh:'Chinois traditionnel (ZH)',cs:'Tchèque (CS)',sk:'Slovaque (SK)',id:'Indonésien (ID)',ar:'Arabe (AR)'};
    const normLocale=value=>String(value||'').trim().toLowerCase().replaceAll('_','-');
    const localeName=value=>localeLabels[normLocale(value)]||String(value||'').toUpperCase();
    const placementLabel=a=>a.profile_position===null?'Profils de restitution · les 3 profils':`Profil ${Number(a.profile_position)+1}`;
    const associationHtml=()=>associations.length?associations.map(a=>`<article class="admin-media-usage-row"><div><strong>${esc(a.theme_title||'Thématique')}</strong><span>${esc(a.chapter_title||'Chapitre')} · ${esc(placementLabel(a))}${a.locale?' · '+esc(localeName(a.locale)):''}</span></div><button class="button button-danger-soft button-small" type="button" data-remove-catalog-media="${a.id}">Retirer</button></article>`).join(''):'<p class="admin-media-usage-empty">Aucune utilisation catalogue. La vidéo n’est héritée automatiquement par aucune campagne.</p>';
    const inheritedHtml=()=>inherited.length?inherited.map(c=>`<article class="admin-media-usage-row is-inherited"><div><strong>${esc(c.campaign_name||c.title||'Campagne')}</strong><span>${esc(c.organization_name||'Client')} · ${esc(c.chapter_title||'Chapitre')} · ${esc(c.status||'')}</span>${c.legacy_history?'<small>Historique importé · hérite du catalogue</small>':'<small>Campagne Studio · hérite du catalogue</small>'}</div><div class="admin-media-usage-row-actions"><span class="admin-media-usage-badge">Héritée</span><button class="button button-danger-soft button-small" type="button" data-exclude-media-project="${c.id}">Exclure</button></div></article>`).join(''):'<p class="admin-media-usage-empty">Aucune campagne n’hérite actuellement de cette vidéo.</p>';
    const excludedHtml=()=>excluded.length?excluded.map(c=>`<article class="admin-media-usage-row is-excluded"><div><strong>${esc(c.campaign_name||c.title||'Campagne')}</strong><span>${esc(c.organization_name||'Client')} · ${esc(c.chapter_title||'Chapitre')} · ${esc(c.status||'')}</span><small>Cette campagne n’affiche pas cette vidéo malgré l’héritage catalogue.</small></div><div class="admin-media-usage-row-actions"><span class="admin-media-usage-badge is-excluded">Exclue</span><button class="button button-secondary button-small" type="button" data-restore-media-project="${c.id}">Réintégrer</button></div></article>`).join(''):'<p class="admin-media-usage-empty">Aucune campagne exclue.</p>';
    const directHtml=()=>direct.length?direct.map(a=>`<article class="admin-media-usage-row"><div><strong>${esc(a.campaign_name||a.project_title||'Campagne')}</strong><span>${esc(a.organization_name||'Client')} · ${esc(a.chapter_title||'Chapitre')} · ${esc(placementLabel(a))}${a.locale?' · '+esc(localeName(a.locale)):''}</span><small>Association spécifique à cette campagne</small></div><button class="button button-danger-soft button-small" type="button" data-remove-project-media="${a.id}" data-project-id="${a.project_id}">Retirer</button></article>`).join(''):'<p class="admin-media-usage-empty">Aucune association spécifique à une campagne.</p>';
    dialog.innerHTML=`<form method="dialog" class="admin-media-usage-shell"><button class="admin-dialog-close" value="cancel" aria-label="Fermer">×</button><p class="eyebrow">Médiathèque · utilisations</p><h2>${esc(video?.title||usage.video?.title||'Vidéo')}</h2><p class="admin-media-delete-lead">Une même vidéo peut être utilisée dans plusieurs thématiques et plusieurs chapitres. Chaque utilisation catalogue est héritée uniquement par les campagnes compatibles avec le chapitre et la langue choisis.</p><div class="admin-media-usage-info"><strong>ⓘ Utilisations multiples</strong><span>Ajoutez autant d’utilisations que nécessaire : par exemple Collègue inclusif, Manager niveau 1 et Manager niveau 2. Une campagne historique n’hérite de la vidéo que si elle est reliée au chapitre sélectionné et possède la langue choisie.</span></div><section><div class="admin-media-usage-section-head"><div><h3>Utilisations catalogue</h3><p>Chaque ligne ci-dessous est une utilisation indépendante de cette vidéo.</p></div><span class="admin-media-usage-count">${associations.length}</span></div><div data-catalog-usage-list>${associationHtml()}</div><div class="admin-media-usage-add-title"><strong>+ Ajouter une utilisation</strong><span>Choisissez d’abord une thématique, puis un chapitre de cette thématique.</span></div><div class="admin-media-usage-add"><label><span>Thématique</span><select data-usage-theme><option value="">Choisir une thématique…</option>${themes.map(theme=>`<option value="${theme.id}">${esc(theme.title)}</option>`).join('')}</select></label><label><span>Chapitre</span><select data-usage-chapter disabled><option value="">Choisir d’abord une thématique…</option></select></label><label><span>Profils concernés</span><select data-usage-profile><option value="">Les 3 profils</option><option value="0">Profil 1</option><option value="1">Profil 2</option><option value="2">Profil 3</option></select></label><label class="admin-media-usage-locale"><span>Langue <small class="admin-media-field-info" title="Seules les campagnes possédant cette langue pourront hériter de la vidéo. Sans langue spécifique, la vidéo est disponible pour toutes les langues compatibles du chapitre.">i</small></span><select data-usage-locale disabled><option value="">Choisir d’abord un chapitre…</option></select></label><button class="button button-primary" type="button" data-add-catalog-usage>Ajouter cette utilisation</button></div></section><section><div class="admin-media-usage-section-head"><div><h3>Campagnes qui héritent de cette vidéo</h3><p>Le calcul tient compte du chapitre associé et de la langue active. Utilisez « Exclure » pour retirer seulement une campagne sans toucher au catalogue.</p></div><span class="admin-media-usage-count">${inherited.length}</span></div><div class="admin-media-usage-scroll" data-inherited-usage-list>${inheritedHtml()}</div></section><section><div class="admin-media-usage-section-head"><div><h3>Campagnes exclues</h3><p>Ces campagnes restent compatibles avec l’utilisation catalogue mais n’affichent pas cette vidéo.</p></div><span class="admin-media-usage-count">${excluded.length}</span></div><div class="admin-media-usage-scroll" data-excluded-usage-list>${excludedHtml()}</div></section><section><div class="admin-media-usage-section-head"><div><h3>Exceptions / associations spécifiques</h3><p>À utiliser seulement lorsqu’une campagne historique doit avoir une utilisation différente du catalogue.</p></div><span class="admin-media-usage-count">${direct.length}</span></div><div data-direct-usage-list>${directHtml()}</div></section><div class="admin-media-delete-actions"><button class="button button-secondary" value="cancel">Fermer</button></div></form>`;
    document.body.appendChild(dialog);dialog.showModal();
    const refresh=async()=>{dialog.close();await openMediaUsageManager(videoId);};
    const themeSelect=dialog.querySelector('[data-usage-theme]'),chapterSelect=dialog.querySelector('[data-usage-chapter]'),profile=dialog.querySelector('[data-usage-profile]'),localeSelect=dialog.querySelector('[data-usage-locale]');
    const selectedTheme=()=>themes.find(theme=>String(theme.id)===String(themeSelect.value));
    const selectedChapter=()=>((selectedTheme()?.chapters)||[]).find(ch=>String(ch.id)===String(chapterSelect.value));
    const syncChapters=()=>{
      const theme=selectedTheme();
      const chapters=(theme?.chapters||[]).slice().sort((a,b)=>Number(a.position||0)-Number(b.position||0)||String(a.title||'').localeCompare(String(b.title||''),'fr'));
      chapterSelect.disabled=!theme;
      chapterSelect.innerHTML=theme?'<option value="">Choisir un chapitre…</option>'+chapters.map(ch=>`<option value="${ch.id}">${esc(ch.title)}</option>`).join(''):'<option value="">Choisir d’abord une thématique…</option>';
      syncLocales();
    };
    const syncLocales=()=>{
    ...
    themeSelect.onchange=syncChapters;chapterSelect.onchange=syncLocales;syncChapters();
    dialog.querySelector('[data-add-catalog-usage]').onclick=async()=>{
      const chapterId=chapterSelect.value;
      if(!themeSelect.value){showError('Choisissez d’abord une thématique.');return;}
      if(!chapterId){showError('Choisissez ensuite un chapitre.');return;}
      const button=dialog.querySelector('[data-add-catalog-usage]');button.disabled=true;button.textContent='Ajout…';
      try{
        await StudioAPI.request('/api/admin/catalog/chapters/'+encodeURIComponent(chapterId)+'/media',{method:'POST',body:JSON.stringify({videoId:Number(videoId),placement:'profile_result',profilePosition:profile.value===''?null:Number(profile.value),locale:localeSelect.value||null,active:true})});
        state.catalogLoaded=false;await refresh();
      }catch(error){button.disabled=false;button.textContent='Ajouter cette utilisation';showError(error.message);}
    };
    dialog.querySelectorAll('[data-exclude-media-project]').forEach(button=>button.onclick=async()=>{
      button.disabled=true;button.textContent='Exclusion…';
      try{await StudioAPI.request('/api/admin/media-library/'+encodeURIComponent(videoId)+'/exclusions',{method:'POST',body:JSON.stringify({projectId:Number(button.dataset.excludeMediaProject)})});await refresh();}
      catch(error){button.disabled=false;button.textContent='Exclure';showError(error.message);}
    });
    dialog.querySelectorAll('[data-restore-media-project]').forEach(button=>button.onclick=async()=>{
      button.disabled=true;button.textContent='Réintégration…';
      try{await StudioAPI.request('/api/admin/media-library/'+encodeURIComponent(videoId)+'/exclusions/'+encodeURIComponent(button.dataset.restoreMediaProject),{method:'DELETE'});await refresh();}
      catch(error){button.disabled=false;button.textContent='Réintégrer';showError(error.message);}
    });
    dialog.querySelectorAll('[data-remove-catalog-media]').forEach(button=>button.onclick=async()=>{button.disabled=true;try{await StudioAPI.request('/api/admin/catalog/media/'+encodeURIComponent(button.dataset.removeCatalogMedia),{method:'DELETE'});state.catalogLoaded=false;await refresh();}catch(error){button.disabled=false;showError(error.message);}});
    dialog.querySelectorAll('[data-remove-project-media]').forEach(button=>button.onclick=async()=>{button.disabled=true;try{await StudioAPI.request('/api/admin/projects/'+encodeURIComponent(button.dataset.projectId)+'/media/'+encodeURIComponent(button.dataset.removeProjectMedia),{method:'DELETE'});await refresh();}catch(error){button.disabled=false;showError(error.message);}});
    dialog.addEventListener('click',e=>{if(e.target===dialog)dialog.close('cancel');});dialog.addEventListener('close',()=>dialog.remove(),{once:true});
  }
  ...
  async function openGlobalMediaDelete(videoId){
    ...
    associations.forEach(a=>{const group=grouped.find(g=>g.theme===a.theme_title&&g.chapter===a.chapter_title);if(group)group.items.push(a);});
    const placementLabel=a=>a.placement==='profile_result'?(a.profile_position===null?'Profils de restitution · les 3 profils':`Profil ${Number(a.profile_position)+1}`):'Après le chapitre';
    const dialog=document.createElement('dialog');dialog.id='admin-media-global-delete-dialog';dialog.className='admin-dialog admin-media-delete-dialog';
    ...
    const inheritedHtml=inherited.length?inherited.map(c=>`<li><strong>${esc(c.campaign_name||c.title||'Campagne')}</strong><span>${esc(c.organization_name||'Client')} · ${esc(c.status||'')}</span><small>${c.legacy_history?'Historique importé · ':''}hérite de l’association catalogue « ${esc(c.chapter_title||'Chapitre')} »</small></li>`).join(''):'<li class="is-empty"><span>Aucune campagne n’hérite de cette ressource.</span></li>';
    const directHtml=direct.length?direct.map(c=>`<li><strong>${esc(c.campaign_name||c.project_title||'Campagne')}</strong><span>${esc(c.organization_name||'Client')} · ${esc(c.chapter_title||'Chapitre')}</span><small>Association spécifique à la campagne</small></li>`).join(''):'<li class="is-empty"><span>Aucune association spécifique à une campagne.</span></li>';
    dialog.innerHTML=`<form method="dialog" class="admin-media-delete-shell"><button class="admin-dialog-close" value="cancel" aria-label="Fermer">×</button><p class="eyebrow">Suppression globale</p><h2>Supprimer définitivement cette ressource ?</h2><p class="admin-media-delete-lead">« ${esc(video?.title||usage.video?.title||'Cette ressource')} » sera supprimée de la médiathèque et de ses associations réelles.</p><div class="admin-media-delete-summary"><span><b>${associations.length}</b> association${associations.length>1?'s':''} catalogue</span><span><b>${direct.length}</b> association${direct.length>1?'s':''} spécifique${direct.length>1?'s':''}</span><span><b>${usage.campaignCount||0}</b> campagne${Number(usage.campaignCount||0)>1?'s':''} impactée${Number(usage.campaignCount||0)>1?'s':''}</span></div><section><h3>Associations catalogue réelles</h3><ul>${usageHtml}</ul></section><section><h3>Campagnes qui héritent du catalogue</h3><ul>${inheritedHtml}</ul></section>${direct.length?`<section><h3>Associations spécifiques aux campagnes</h3><ul>${directHtml}</ul></section>`:''}${inherited.length?`<div class="admin-media-delete-warning"><strong>Conséquence de l’héritage</strong><span>Ces campagnes ne contiennent pas une copie indépendante de la vidéo : elles l’affichent parce qu’elles utilisent un chapitre associé dans le catalogue. La suppression de l’association catalogue fera donc disparaître la vidéo de ces campagnes.</span></div>`:''}<div class="admin-media-delete-final"><strong>Cette action est irréversible.</strong><span>La vidéo elle-même sera supprimée de la médiathèque, ainsi que ${associations.length} association${associations.length>1?'s':''} catalogue et ${direct.length} association${direct.length>1?'s':''} spécifique${direct.length>1?'s':''}. Les autres contenus des campagnes restent inchangés.</span></div><div class="admin-media-delete-actions"><button class="button button-ghost" value="cancel">Annuler</button><button class="button studio-modal-danger" type="button" data-confirm-global-media-delete>Supprimer partout</button></div></form>`;
    document.body.appendChild(dialog);dialog.showModal();dialog.addEventListener('click',e=>{if(e.target===dialog)dialog.close('cancel');});
    const confirm=dialog.querySelector('[data-confirm-global-media-delete]');confirm.onclick=async()=>{confirm.disabled=true;confirm.textContent='Suppression…';try{await StudioAPI.request('/api/admin/media-library/'+encodeURIComponent(videoId)+'?global=1',{method:'DELETE'});dialog.close('deleted');await loadMediaLibrary();state.catalogLoaded=false;}catch(error){confirm.disabled=false;confirm.textContent='Supprimer partout';showError(error.message);}};
  ...
  const libraryCountryLabel=code=>({EU:'Europe',FR:'France',BE:'Belgique',CH:'Suisse',CA:'Canada',LU:'Luxembourg',ES:'Espagne',DE:'Allemagne',AR:'Argentine',AT:'Autriche',BR:'Brésil',CL:'Chili',CN:'Chine',KR:'Corée du Sud',DK:'Danemark',US:'États-Unis',HK:'Hong Kong',JP:'Japon',MX:'Mexique',NO:'Norvège',PA:'Panama',PT:'Portugal',SE:'Suède',TW:'Taïwan',UY:'Uruguay',IT:'Italie',GB:'Royaume-Uni',NL:'Pays-Bas',PL:'Pologne',RO:'Roumanie',RU:'Russie',TR:'Turquie',BG:'Bulgarie'}[String(code||'').toUpperCase()]||String(code||''));
  ...
  const libraryCountryOptions=['EU','AR','AT','BR','CA','CL','CN','KR','DK','ES','US','FR','HK','JP','MX','NO','PA','PT','SE','CH','TW','UY','DE'];
  function renderThemeCountryEditor(form){
    ...
    select.innerHTML='<option value="">Ajouter un périmètre…</option>'+libraryCountryOptions.filter(c=>!codes.includes(c)).map(c=>`<option value="${c}">${esc(libraryCountryLabel(c))}</option>`).join('');
...
Les tags existants seront conservés. La bibliothèque complémentaire Sexisme est incluse. Les autres bibliothèques complémentaires seront intégrées plus tard à partir de vos fichiers Excel ou Word.`;const confirmed=await StudioModal.confirm({eyebrow:'Catalogue Me&YouToo',title:'Intégrer les tags proposés ?',message,type:'info',cancelLabel:'Annuler',confirmLabel:'Intégrer les tags'});if(!confirmed){autoTag.disabled=false;autoTag.textContent='Analyser et intégrer les tags';return;}autoTag.textContent='Intégration…';const result=await StudioAPI.request('/api/admin/catalog/tagging-apply',{method:'POST',body:'{}'});await refreshLibrary();await StudioModal.alert({eyebrow:'Catalogue Me&YouToo',title:'Tags intégrés',message:`${result.updated||0} situation${Number(result.updated||0)>1?'s':''} enrichie${Number(result.updated||0)>1?'s':''}, base et bibliothèque complémentaire Sexisme incluses.`,type:'success',confirmLabel:'Fermer'});}catch(error){showError(error.message);}finally{const b=$('[data-auto-tag-catalog]');if(b){b.disabled=false;b.textContent='Analyser et intégrer les tags';}}};
  ...
  ['org-search','org-sector-filter','org-theme-filter','org-locale-filter','org-scope-filter','org-folder-filter','org-sort'].forEach(id=>$('#'+id)?.addEventListener(id==='org-search'?'input':'change',()=>{if(id==='org-folder-filter')state.clientFolderFilter=$('#org-folder-filter').value||'all';if(['org-theme-filter','org-locale-filter','org-scope-filter'].includes(id)){renderLocaleFilter();renderLanguageCountFilter();renderScopeFilter();}renderChips();renderClientFolderBar();renderOrganizations();}));
  $('#org-language-count-options')?.addEventListener('change',e=>{if(!e.target.matches('[data-language-count]'))return;renderLocaleFilter();renderLanguageCountFilter();renderScopeFilter();renderChips();renderClientFolderBar();renderOrganizations();});
  $('[data-clear-language-counts]')?.addEventListener('click',()=>{$$('[data-language-count]').forEach(input=>input.checked=false);renderLocaleFilter();renderLanguageCountFilter();renderScopeFilter();renderChips();renderClientFolderBar();renderOrganizations();});
