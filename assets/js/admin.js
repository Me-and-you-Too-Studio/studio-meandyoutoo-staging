...
const normalizedStatus=p=>p.status==='configuration_submitted'?'review_pending':p.status==='completed'?'unpublished':p.status;
  ...
  function renderOrganizations(){
    ...
    function populateAdminCampaignFilters(rows){
      ...
      // Dans le filtre, on n'affiche qu'une occurrence par libellé pour éviter les doublons.
      const scopeByLabel=new Map();
      rawScopes.forEach(code=>{const label=projectCountryLabel(code),key=String(label||'').trim().toLocaleLowerCase('fr');if(label&&!scopeByLabel.has(key))scopeByLabel.set(key,{code,label});});
      ...
      const detailedScopeOptions=detailedScopes.length?'<optgroup label="Pays">'+detailedScopes.map(item=>`<option value="${esc(item.code)}">${esc(item.label)}</option>`).join('')+'</optgroup>':'';
    ...
    function renderCampaigns(){
      ...
      const rows=allRows.filter(p=>{
        ...
        const franceOnly=countries.length===1&&projectCountryLabel(countries[0])==='France';
        const scopeMatch=!scope||(scope==='__france__'?franceOnly:scope==='__international__'?(countries.length>0&&!franceOnly):countries.includes(scope));
        return adminCampaignQuickMatch(p,state.campaignQuickFilter)&&statusMatch&&(!theme||p.theme_title===theme)&&adminCampaignPeriodMatch(p,period)&&scopeMatch&&(!locale||locales.includes(locale))&&(!client||String(p.organizationId)===String(client))&&(!q||haystack.includes(q));
      });
