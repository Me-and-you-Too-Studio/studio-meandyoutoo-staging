...
const normalizedStatus=p=>p.status==='configuration_submitted'?'review_pending':p.status==='completed'?'unpublished':p.status;
  ...
  function renderOrganizations(){
    ...
    function adminCampaignFinishedStatus(p){return adminCampaignLifecycle(typeof p==='object'?p:{status:p})==='finished';}
    ...
    function adminCampaignQuickMatch(p,key){if(!key||key==='all')return true;if(key==='endingSoon')return adminCampaignEndingSoon(p);if(key==='results')return adminCampaignHasResults(p);return adminCampaignLifecycle(p)===key;}
    ...
    function renderAdminCampaignKpis(rows){
      const counts={all:rows.length,draft:rows.filter(p=>adminCampaignLifecycle(p)==='draft').length,review:rows.filter(p=>adminCampaignLifecycle(p)==='review').length,ongoing:rows.filter(p=>adminCampaignLifecycle(p)==='ongoing').length,finished:rows.filter(p=>adminCampaignLifecycle(p)==='finished').length};
    ...
    function renderCampaigns(){
      ...
      $('#admin-campaigns').innerHTML=rows.map(p=>{const countries=projectCountries(p),locales=projectLocales(p),lifecycle=adminCampaignLifecycle(p),lifecycleLabel={draft:'Brouillon',review:'À traiter',scheduled:'Programmée',ongoing:'En cours',finished:'Terminée',unpublished:'Dépubliée',archived:'Archivée'}[lifecycle]||labels[p.status]||p.status,signals=[adminCampaignEndingSoon(p)?'<span class="admin-campaign-signal is-ending">🔴 Fin proche</span>':'',adminCampaignHasResults(p)?'<span class="admin-campaign-signal is-results">📊 Résultats disponibles</span>':''].filter(Boolean).join(''),meta=[...countries.slice(0,4).map(c=>`<span>${esc(projectCountryLabel(c))}</span>`),...locales.slice(0,5).map(l=>`<span>${esc(localeLabel(l))}</span>`)].join('');return `<article class="admin-campaign-card" data-project-card="${p.id}"><div class="admin-campaign-card-main"><div class="admin-campaign-card-badges"><span class="badge badge-status">${esc(lifecycleLabel)}</span>${signals}</div><h3>${esc(p.campaign_name||p.title||'Sans nom')}</h3><p><strong>${esc(p.organizationName)}</strong> · ${esc(p.theme_title||'Thématique non renseignée')}</p>${meta?`<div class="admin-campaign-meta-tags">${meta}</div>`:''}</div><dl><div><dt>Étape</dt><dd>${esc(p.current_step||'—')}</dd></div><div><dt>Dernière modification</dt><dd>${dateTime(p.updated_at)}</dd></div><div><dt>Dates de diffusion</dt><dd>${date(p.launch_date)} → ${date(p.close_date)}</dd></div></dl><a class="button button-secondary" href="client.html?projectId=${p.id}">Ouvrir dans le dossier client</a>${adminCommentPanel('project',p.id)}</article>`;}).join('')||'<div class="card admin-empty">Aucune campagne ne correspond à ces filtres.</div>';
