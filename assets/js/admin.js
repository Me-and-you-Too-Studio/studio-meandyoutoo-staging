...
const normalizedStatus=p=>p.status==='configuration_submitted'?'review_pending':p.status==='completed'?'unpublished':p.status;
  ...
  function libraryLocalizationBadges(entity){
    ...
    const locales=[...new Set(['fr',...generic.map(t=>String(t.locale||'').toLowerCase()).filter(Boolean)])];
    const countries=[...new Set(libraryCountryVariants(entity).map(t=>String(t.country_code||'').trim()).filter(Boolean))];
    return `<span class="admin-library-localization-badges"><span class="admin-library-localization-chip is-language">Langues ${locales.map(libraryLocaleLabel).join(' · ')}</span>${countries.map(c=>`<span class="admin-library-localization-chip is-country">Périmètre culturel : ${esc(c)}</span>`).join('')}</span>`;
  }
  function libraryLocalizationDetails(entity,entityType){
    const generic=libraryGenericTranslations(entity);
    const variants=libraryCountryVariants(entity);
    if(!generic.length&&!variants.length)return `<p class="admin-library-localization-empty">Aucune traduction ou variante culturelle enregistrée.</p>`;
    const rowHtml=(row,isVariant=false)=>{
      ...
      let body='';
      if(entityType==='profile')body=`<div><b>Titre</b><p>${esc(c.title||'—')}</p></div><div><b>Résumé</b><p>${esc(c.summary||'—')}</p></div><div><b>Texte détaillé</b><p>${esc(c.content||'—')}</p></div>`;
      else body=`<div><b>Texte</b><p>${esc(c.content||c.title||'—')}</p></div>`;
      return `<details class="admin-library-localization-row ${isVariant?'is-cultural':''}"><summary><strong>${isVariant?'Variante culturelle':'Traduction'} ${locale}${country}</strong><span>${isVariant?'Version spécifique au périmètre':'Version linguistique'}</span></summary><div class="admin-library-localization-copy">${body}</div></details>`;
    };
