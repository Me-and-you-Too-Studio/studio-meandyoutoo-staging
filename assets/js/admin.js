...
const normalizedStatus=p=>p.status==='configuration_submitted'?'review_pending':p.status==='completed'?'unpublished':p.status;
  ...
  function libraryLocalizationBadges(entity){
    ...
    const countries=[...new Set([...(entity?.cultural_legal_scope?[String(entity.cultural_legal_scope)]:[]),...libraryCountryVariants(entity).map(t=>String(t.country_code||'').trim()).filter(Boolean)])];
    return `<span class="admin-library-localization-badges"><span class="admin-library-localization-chip is-language">Langues disponibles : ${locales.map(libraryLocaleLabel).join(' · ')}</span>${countries.map(c=>`<span class="admin-library-localization-chip is-country">Périmètre culturel et légal : ${esc(c)}</span>`).join('')}</span>`;
  }
  ...
  function bindLibraryActions(){
    ...
    $$('[data-save-library-translation]').forEach(button=>button.onclick=async e=>{
      ...
      try{await StudioAPI.request('/api/admin/catalog/translations/'+button.dataset.saveLibraryTranslation,{method:'PATCH',body:JSON.stringify({content})});await refreshLibrary();renderLibraryAdmin();}
