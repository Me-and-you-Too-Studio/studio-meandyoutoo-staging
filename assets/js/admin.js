...
const normalizedStatus=p=>p.status==='configuration_submitted'?'review_pending':p.status==='completed'?'unpublished':p.status;
  ...
  async function openMediaUsageManager(videoId){
    ...
    const syncProfile=()=>{profile.disabled=placement.value!=='profile_result';if(profile.disabled)profile.value='';};
    themeSelect.onchange=syncChapters;chapterSelect.onchange=syncLocales;placement.onchange=syncProfile;syncProfile();syncChapters();
    dialog.querySelector('[data-add-catalog-usage]').onclick=async()=>{
      ...
      try{
        await StudioAPI.request('/api/admin/catalog/chapters/'+encodeURIComponent(chapterId)+'/media',{method:'POST',body:JSON.stringify({videoId:Number(videoId),placement:placement.value,profilePosition:profile.value===''?null:Number(profile.value),locale:localeSelect.value||null,active:true})});
