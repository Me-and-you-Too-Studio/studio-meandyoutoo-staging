...
const normalizedStatus=p=>p.status==='configuration_submitted'?'review_pending':p.status==='completed'?'unpublished':p.status;
  ...
  function globalPickerHtml(targetChapter){
    ...
    return `<section class="admin-global-catalog-picker" data-global-picker="${targetChapter.id}"><div class="admin-global-picker-head"><div><strong>Catalogue global Me&amp;YouToo</strong><p>Ces situations existent déjà dans d’autres autodiagnostics. Vous les réutilisez ici sans les dupliquer.</p></div><button type="button" class="button button-ghost button-small" data-close-global-picker>Fermer</button></div><div class="admin-global-picker-destination"><span>Destination : <strong>${pickerPlacementLabel}</strong></span><span class="admin-info-dot" tabindex="0" title="${esc(pickerPlacementHelp)} La situation source reste inchangée dans sa thématique d’origine.">i</span></div><div class="admin-global-picker-help"><span class="admin-info-dot" tabindex="0" title="Vous êtes dans une zone précise de la thématique. La destination est donc déjà définie ici : vous ne choisissez plus Base ou Bibliothèque sur chaque situation. Le client n’a jamais accès au catalogue global.">i</span><span>Filtrez le catalogue, choisissez les langues actives, puis réutilisez la situation dans cette destination.</span></div><div class="admin-global-picker-filters"><label><span>Thématique d’origine <span class="admin-info-dot" tabindex="0" title="Filtre les situations selon l’autodiagnostic dans lequel elles ont été créées à l’origine.">i</span></span><select data-global-picker-theme><option value="">Toutes les thématiques</option>${sourceThemes.map(t=>`<option value="${t.id}" ${themeFilter===String(t.id)?'selected':''}>${esc(t.title)}</option>`).join('')}</select></label><label><span>Chapitre d’origine <span class="admin-info-dot" tabindex="0" title="Affiche uniquement les situations du chapitre source choisi. Ce filtre dépend de la thématique d’origine sélectionnée.">i</span></span><select data-global-picker-source-chapter><option value="">Tous les chapitres</option>${sourceChapters.map(ch=>`<option value="${ch.id}" ${sourceChapterFilter===String(ch.id)?'selected':''}>${esc(ch.title)}</option>`).join('')}</select></label></div><label class="admin-global-picker-search"><span>Rechercher par mot-clé, contexte ou texte <span class="admin-info-dot" tabindex="0" title="La recherche porte sur le texte de la situation, ses tags contextuels et ses mots-clés : réunion, chantier, industrie, harcèlement, management, etc.">i</span></span><input type="search" data-global-picker-search value="${esc(state.libraryGlobalPickerQuery||'')}" placeholder="Ex. harcèlement, réunion, chantier, industrie, management…"></label><div class="admin-global-picker-results">${rows.length?rows.map(row=>`<article class="admin-global-picker-card" data-global-situation-card="${row.situation.id}"><div><div class="admin-global-picker-source"><span>${esc(row.theme.title)}</span><b>›</b><span>${esc(row.chapter.title)}</span><span class="admin-info-dot" tabindex="0" title="Cette provenance reste la source officielle de la situation. La réutilisation ici ne crée pas de copie.">i</span></div><p>${esc(libraryRealContent(row.situation))}</p>${situationTagHtml(row.situation)}${languageChoices(row)}</div><div class="admin-global-picker-actions"><button type="button" class="button button-primary button-small" data-link-global-situation="${row.situation.id}" data-target-chapter="${targetChapter.id}" data-placement="${pickerPlacement}">Réutiliser cette situation</button></div></article>`).join(''):`<p class="admin-library-empty-line">Aucune situation ne correspond à ces filtres.</p>`}</div></section>`;
  }
  ...
  function linkedSituationLocaleEditor(si){
    ...
    return `<div class="admin-global-picker-locales admin-linked-locale-editor"><div class="admin-global-picker-locales-title"><strong>Langues actives dans cet autodiagnostic</strong><span class="admin-info-dot" tabindex="0" title="Vous ne modifiez pas les traductions de la situation source. Vous choisissez seulement quelles langues sont actives dans cet autodiagnostic.">i</span></div><div class="admin-global-picker-locale-chips">${ordered.map(loc=>`<label><input type="checkbox" data-linked-active-locale value="${esc(loc)}" ${active.has(loc)?'checked':''}> <span>${esc(libraryLocaleLabel(loc))}</span></label>`).join('')}</div><small>Les autres traductions restent disponibles dans la situation d’origine et peuvent être réactivées plus tard.</small></div>`;
  }
  ...
  function bindLibraryActions(){
    ...
    $$('[data-open-global-picker]').forEach(b=>b.onclick=e=>{e.stopPropagation();state.libraryGlobalPickerChapterId=String(b.dataset.openGlobalPicker);state.libraryGlobalPickerPlacement=b.dataset.pickerPlacement==='library'?'library':'base';state.libraryGlobalPickerQuery='';state.libraryGlobalPickerThemeId='';state.libraryGlobalPickerSourceChapterId='';state.libraryExpandedChapters.add(String(b.dataset.openGlobalPicker));renderLibraryAdmin();setTimeout(()=>document.querySelector('[data-global-picker-search]')?.focus(),0);});
    ...
    $$('[data-unlink-situation-placement]').forEach(b=>b.onclick=async()=>{if(!confirm('Retirer cette situation de cet autodiagnostic ? La situation source restera dans le Catalogue Me&YouToo.'))return;try{await StudioAPI.request('/api/admin/catalog/situation-placements/'+b.dataset.unlinkSituationPlacement,{method:'DELETE'});await refreshLibrary();renderLibraryAdmin();}catch(error){showError(error.message);}});
