...
const normalizedStatus=p=>p.status==='configuration_submitted'?'review_pending':p.status==='completed'?'unpublished':p.status;
  ...
  function globalPickerHtml(targetChapter){
    ...
    return `<section class="admin-global-catalog-picker" data-global-picker="${targetChapter.id}"><div class="admin-global-picker-head"><div><strong>Catalogue global Me&amp;YouToo</strong><p>Recherche transversale réservée aux admins. Le client ne verra que les situations rattachées à la bibliothèque complémentaire de son autodiagnostic.</p></div><button type="button" class="button button-ghost button-small" data-close-global-picker>Fermer</button></div><label class="admin-global-picker-search"><span>Rechercher par mot-clé, contexte, thématique ou texte</span><input type="search" data-global-picker-search value="${esc(state.libraryGlobalPickerQuery||'')}" placeholder="Ex. harcèlement, réunion, chantier, industrie, management…"></label><div class="admin-global-picker-results">${rows.length?rows.map(row=>`<article class="admin-global-picker-card"><div><div class="admin-global-picker-source"><span>${esc(row.theme.title)}</span><b>›</b><span>${esc(row.chapter.title)}</span></div><p>${esc(libraryRealContent(row.situation))}</p>${situationTagHtml(row.situation)}</div><div class="admin-global-picker-actions"><button type="button" class="button button-secondary button-small" data-link-global-situation="${row.situation.id}" data-target-chapter="${targetChapter.id}" data-placement="library">+ Bibliothèque complémentaire</button><button type="button" class="button button-primary button-small" data-link-global-situation="${row.situation.id}" data-target-chapter="${targetChapter.id}" data-placement="base">+ Base du thème</button></div></article>`).join(''):`<p class="admin-library-empty-line">Aucune situation ne correspond à cette recherche.</p>`}</div></section>`;
  }
  ...
  function bindLibraryActions(){
    ...
    $$('[data-open-global-picker]').forEach(b=>b.onclick=e=>{e.stopPropagation();state.libraryGlobalPickerChapterId=String(b.dataset.openGlobalPicker);state.libraryGlobalPickerQuery='';state.libraryExpandedChapters.add(String(b.dataset.openGlobalPicker));renderLibraryAdmin();setTimeout(()=>document.querySelector('[data-global-picker-search]')?.focus(),0);});
    $$('[data-close-global-picker]').forEach(b=>b.onclick=()=>{state.libraryGlobalPickerChapterId=null;state.libraryGlobalPickerQuery='';renderLibraryAdmin();});
