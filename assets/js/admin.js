...
const normalizedStatus=p=>p.status==='configuration_submitted'?'review_pending':p.status==='completed'?'unpublished':p.status;
  ...
  function syncMigrationLotName(){
    const input=$('#migration-source-name'),jsonInput=$('#migration-source-json');
    if(!input||!jsonInput)return;
    const data=jsonInput._migrationData||null;if(!data)return;
    const scope=$('#migration-scope')?.value||data.suggestedScope||'catalog';
    const surveyId=String($('#migration-source-survey-id')?.value||data.sourceSurveyId||'').trim();
    const title=String(data.diagnosticTitle||'Migration historique').trim();
    let owner='Me&YouToo';
    if(scope==='client'){
      const sel=$('#migration-org'),opt=sel?.selectedOptions?.[0];
      owner=String(opt?.textContent||'').trim();
      if(!sel?.value||!owner||owner==='Choisir un client')owner=String(data.sourceCustomerName||'Client').trim()||'Client';
    }
    input.value=`${owner} — ${title}${surveyId?` — #${surveyId}`:''}`;
  }
