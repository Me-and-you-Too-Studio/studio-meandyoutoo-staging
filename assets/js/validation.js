(()=>{
  ...
  function hideSubmittedPreparation(){
    document.querySelector('nav.steps')?.setAttribute('hidden','');
    document.querySelector('.studio-process-card')?.setAttribute('hidden','');
    document.querySelector('.validation-summary')?.setAttribute('hidden','');
    $('validation-credit')?.setAttribute('hidden','');
    document.querySelector('[data-validation-check]')?.closest('section')?.setAttribute('hidden','');
    $('submit-project')?.closest('.top-actions')?.setAttribute('hidden','');
    document.querySelector('.step-time-badge')?.setAttribute('hidden','');
    const alert=$('validation-alert');if(alert){alert.hidden=true;alert.textContent='';}
  }
  async function openAdjustmentDialog(differences,call){
    ...
    const targets=differences.map((row,index)=>({key:`difference:${index}`,label:row[0],clientText:plain(row[1]),meaytText:plain(row[2]),category:/profil/i.test(row[0])?'profile':/introduction/i.test(row[0])?'intro':/date|campagne|titre répondants/i.test(row[0])?'settings':'situation'}));
    dialog.innerHTML=`<form method="dialog"><button class="admin-dialog-close" value="cancel" aria-label="Fermer">×</button><p class="eyebrow">Demande ciblée</p><h2>Que souhaitez-vous faire ajuster ?</h2><p>Votre configuration reste verrouillée. Me&YouToo reprendra uniquement les éléments que vous signalez.</p><label class="field"><span>Nature de la demande</span><select id="adjust-category"><option value="">Choisir…</option><option value="situation">Une situation ou une réponse</option><option value="profile">Un profil</option><option value="intro">L’introduction</option><option value="settings">Les dates ou les titres</option><option value="multiple">Plusieurs éléments</option><option value="other">Autre demande</option></select></label><fieldset class="adjust-targets" id="adjust-targets"><legend>Contenu à faire reprendre</legend><p class="adjust-target-help">Sélectionnez le texte précis concerné par votre demande, même si Me&YouToo ne l’a pas encore modifié.</p>${targets.map((t,index)=>`<label class="adjust-target-card" data-category="${t.category}"><span class="adjust-target-choice"><input type="checkbox" value="${esc(t.key)}" data-label="${esc(t.label)}" data-target-index="${index}"> <strong>${esc(t.label)}</strong></span><span class="adjust-version is-client"><small>Votre version</small>${esc(t.clientText)||'—'}</span><span class="adjust-version is-meayt"><small>Version proposée par Me&YouToo</small><span class="review-word-diff">${wordDiff(t.clientText,t.meaytText,true)}</span></span></label>`).join('')||'<p>Aucune différence détaillée disponible.</p>'}</fieldset><label class="field"><span>Expliquez le résultat attendu <small>(10 caractères minimum)</small></span><textarea id="adjust-message" rows="5" placeholder="Ex. : conserver la formulation du client dans la situation 3, mais remplacer le dernier terme…"></textarea></label><div class="adjust-summary" id="adjust-summary">Sélectionnez une catégorie et le contenu à faire reprendre.</div><div class="top-actions"><button class="button button-ghost" value="cancel">Annuler</button><button class="button button-primary" id="adjust-send" type="button">Vérifier et envoyer</button></div></form>`;
    document.body.append(dialog);const category=$('adjust-category'),targetBox=$('adjust-targets'),summaryBox=$('adjust-summary');
    const update=()=>{const value=category.value,filtered=Boolean(value&&!['multiple','other'].includes(value)),visible=[];targetBox.querySelectorAll('label').forEach(label=>{label.hidden=filtered&&label.dataset.category!==value;const input=label.querySelector('input');if(label.hidden&&input)input.checked=false;else if(input)visible.push(input);});if(filtered&&visible.length===1)visible[0].checked=true;const selected=[...targetBox.querySelectorAll('input:checked')].filter(x=>!x.closest('label').hidden);summaryBox.innerHTML=selected.length?`<strong>${selected.length} élément${selected.length>1?'s':''} signalé${selected.length>1?'s':''}</strong><br>${selected.map(x=>esc(x.dataset.label)).join('<br>')}`:'Sélectionnez précisément ce que Me&YouToo doit reprendre.';};category.onchange=update;targetBox.onchange=update;
    $('adjust-send').onclick=async()=>{const message=$('adjust-message').value.trim(),selected=[...targetBox.querySelectorAll('input:checked:not([hidden])')].filter(x=>!x.closest('label').hidden),cat=category.value;if(!cat)return window.StudioModal.alert({title:'Choisissez la nature de la demande',message:'Cela permettra à Me&YouToo de traiter votre retour sans ambiguïté.',type:'info'});if(cat!=='other'&&!selected.length)return window.StudioModal.alert({title:'Sélectionnez au moins un élément',message:'Cochez la ou les corrections concernées.',type:'info'});if(message.length<10)return window.StudioModal.alert({title:'Précisez le résultat attendu',message:'Votre commentaire doit contenir au moins 10 caractères.',type:'info'});const chosen=selected.map(x=>{const target=targets[Number(x.dataset.targetIndex)]||{};return {key:x.value,label:x.dataset.label,clientText:target.clientText||'',meaytText:target.meaytText||''};}),ok=await window.StudioModal.confirm({eyebrow:'Confirmation',title:'Envoyer cette demande d’ajustement ?',message:`${chosen.length||'Aucun'} élément ciblé. Votre commentaire : « ${message} »`,confirmLabel:'Envoyer à Me&YouToo',cancelLabel:'Relire ma demande'});if(ok){dialog.close();await call(`/api/projects/${projectId}/review/request-adjustment`,{message,category:cat,targets:chosen});}};dialog.showModal();
  }
  function reviewPanel(){
    ...
    const adjustmentTargets=[['Nom de campagne',beforeProject.campaign_name||project.campaign_name,project.campaign_name],['Titre répondants',beforeProject.respondent_title||project.respondent_title,project.respondent_title],['Introduction du diagnostic',beforeProject.introduction_html||project.introduction_html,project.introduction_html]];
    afterSituations.forEach(now=>{const old=beforeSituationMap.get(String(now.id))||now,base=`Partie ${now.chapterNumber} · ${now.chapter} · Situation ${now.situationNumber}`;adjustmentTargets.push([`${base} · Texte`,old.content||now.content,now.content]);(now.answers||[]).forEach((answer,ai)=>{const previous=(old.answers||[]).find(item=>String(item.id)===String(answer.id))||answer;adjustmentTargets.push([`${base} · Réponse ${ai+1}`,previous.content||answer.content,answer.content]);});});
    afterProfiles.forEach(now=>{const old=beforeProfileMap.get(String(now.id))||now,base=`Partie ${now.chapterNumber} · ${now.chapter} · Profil ${now.profileNumber}/3 · Niveau ${levels[now.profileNumber-1]||now.color||'non défini'}`;[['Titre','title'],['Résumé','summary'],['Contenu détaillé','content']].forEach(([label,key])=>adjustmentTargets.push([`${base} · ${label}`,old[key]||now[key],now[key]]));});
    const comparisonTable=(title,description,rows,leftTitle,rightTitle,rightClass,adminColors=false)=>rows.length?`<div class="review-diff"><div class="review-diff-head"><div><h3>${esc(title)}</h3><p>${esc(description)}</p></div></div><div class="review-diff-scroll"><table><thead><tr><th>Élément contrôlé</th><th>${esc(leftTitle)}</th><th class="${rightClass}">${esc(rightTitle)}</th></tr></thead><tbody>${rows.map(row=>`<tr><th>${isAdmin&&row[3]?`<a class="review-edit-target" href="${esc(row[3])}" title="Ouvrir directement cet élément">${esc(row[0])}<span>Modifier →</span></a>`:esc(row[0])}</th><td>${esc(row[1])}</td><td class="${rightClass} review-word-diff ${adminColors?'is-admin-diff':''}">${wordDiff(row[1],row[2],adminColors)}</td></tr>`).join('')}</tbody></table></div></div>`:'';
    ...
    const adjustment=project.status==='in_review'?[...reviewEvents].find(e=>e.event_type==='client_adjustment_requested'):null,meta=adjustment?.metadata||{},adjustmentBanner=adjustment?`<div class="review-adjustment-pending"><strong>↻ Ajustement demandé par le client</strong><p>${esc(adjustment.summary)}</p>${meta.targets?.length?`<div class="adjustment-review-list">${meta.targets.map(t=>`<article><strong>${esc(t.label)}</strong>${t.clientText?`<p><small>Version du client</small>${esc(t.clientText)}</p>`:''}${t.meaytText?`<p><small>Version Me&YouToo contestée</small>${esc(t.meaytText)}</p>`:''}</article>`).join('')}</div>`:''}</div>`:'';
    const colorLegend=`<div class="review-color-legend" aria-label="Légende des modifications"><span class="is-removed"><i></i><strong>Rouge</strong> : supprimé</span><span class="is-added"><i></i><strong>Vert</strong> : ajouté par le client</span><span class="is-meayt"><i></i><strong>Violet</strong> : Me&YouToo</span></div>`;
    root.innerHTML=`<p class="eyebrow">Contrôle qualité</p><h2 class="section-title">Relecture Me&YouToo</h2><div class="review-status ${adjustment?'is-adjustment':''}"><span>Statut actuel</span><strong>${adjustment?'Ajustement demandé par le client':status}</strong></div>${colorLegend}${adjustmentBanner}${project.status==='ready_to_publish'?`<p class="review-ready">✓ La relecture est terminée. La campagne peut être programmée ou publiée.</p>`:''}${clientDiffHtml}${adminDiffHtml}${clientDiffHtml||adminDiffHtml?`<div class="review-color-legend is-bottom" aria-label="Rappel de la légende des modifications">${colorLegend.replace(/^<div[^>]*>|<\/div>$/g,'')}</div>`:''}${actions}${history}`;
