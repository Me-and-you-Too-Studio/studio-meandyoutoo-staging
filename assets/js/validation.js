(()=>{
  ...
  function reviewPanel(){
    ...
    const addClientDiff=(label,reference,client)=>{if(String(reference??'')!==String(client??''))clientDifferences.push([label,String(reference??'—'),String(client??'—')]);};
    const addAdminDiff=(label,client,current)=>{if(String(client??'')!==String(current??''))adminDifferences.push([label,String(client??'—'),String(current??'—')]);};
    addClientDiff('Nom de campagne',project.base_title,beforeProject.campaign_name);addClientDiff('Titre répondants',project.respondent_title_default,beforeProject.respondent_title);addClientDiff('Introduction du diagnostic',project.theme_introduction_html,beforeProject.introduction_html);
    addAdminDiff('Nom de campagne',beforeProject.campaign_name,project.campaign_name);addAdminDiff('Titre répondants',beforeProject.respondent_title,project.respondent_title);addAdminDiff('Introduction du diagnostic',beforeProject.introduction_html,project.introduction_html);addAdminDiff('Date de lancement',String(beforeProject.launch_date||'').slice(0,10),String(project.launch_date||'').slice(0,10));addAdminDiff('Date de clôture',String(beforeProject.close_date||'').slice(0,10),String(project.close_date||'').slice(0,10));
    ...
    beforeSituationMap.forEach((s,id)=>{const now=afterSituationMap.get(id),label=`Partie ${s.chapterNumber} · ${s.chapter} · Situation ${s.situationNumber}`;if(now){addClientDiff(`${label} · Texte`,now.original_content,s.content);(s.answers||[]).forEach((answer,ai)=>{const original=(now.original_answers||[]).find(a=>String(a.id)===String(answer.id));if(original)addClientDiff(`${label} · Réponse ${ai+1}`,original.content,answer.content);});}if(!now)adminDifferences.push([`${label} · supprimée`,s.content,'Supprimée']);else{addAdminDiff(`${label} · Texte`,s.content,now.content);(s.answers||[]).forEach((answer,ai)=>{const current=(now.answers||[]).find(a=>String(a.id)===String(answer.id));if(current)addAdminDiff(`${label} · Réponse ${ai+1}`,answer.content,current.content);});}});afterSituationMap.forEach((s,id)=>{if(!beforeSituationMap.has(id))adminDifferences.push([`Partie ${s.chapterNumber} · ${s.chapter} · Situation ${s.situationNumber} · ajoutée`,'Absente',s.content]);});
    const beforeProfiles=beforeChapters.flatMap((ch,ci)=>(ch.profiles||[]).map((x,pi)=>({...x,chapter:ch.title,chapterNumber:ci+1,profileNumber:pi+1}))),afterProfiles=currentChapters.flatMap((ch,ci)=>(ch.profiles||[]).map((x,pi)=>({...x,chapter:ch.title,chapterNumber:ci+1,profileNumber:pi+1}))),beforeProfileMap=new Map(beforeProfiles.map(x=>[String(x.id),x])),levels=['rouge','jaune','vert'];afterProfiles.forEach(now=>{const old=beforeProfileMap.get(String(now.id));if(!old)return;const base=`Partie ${now.chapterNumber} · ${now.chapter} · Profil ${now.profileNumber}/3 · Niveau ${levels[now.profileNumber-1]||now.color||'non défini'}`;[['Titre','title','original_title'],['Résumé','summary','original_summary'],['Contenu détaillé','content','original_content']].forEach(([label,key,originalKey])=>{addClientDiff(`${base} · ${label}`,now[originalKey],old[key]);addAdminDiff(`${base} · ${label}`,old[key],now[key]);});});
    const comparisonTable=(title,description,rows,leftTitle,rightTitle,rightClass)=>rows.length?`<div class="review-diff"><div class="review-diff-head"><div><h3>${esc(title)}</h3><p>${esc(description)}</p></div></div><div class="review-diff-scroll"><table><thead><tr><th>Élément contrôlé</th><th>${esc(leftTitle)}</th><th class="${rightClass}">${esc(rightTitle)}</th></tr></thead><tbody>${rows.map(row=>`<tr><th>${esc(row[0])}</th><td>${esc(row[1])}</td><td class="${rightClass}">${esc(row[2])}</td></tr>`).join('')}</tbody></table></div></div>`:'';
    const clientDiffHtml=!hasReviewSnapshot?'<p class="review-no-diff">La comparaison n’est pas disponible pour cette ancienne transmission.</p>':clientDifferences.length?comparisonTable('Adaptations transmises par le client','Vérifiez directement chaque modification avant de décider si une correction est nécessaire.',clientDifferences,'Référentiel Me&YouToo','Version transmise par le client','review-client-version'):'<p class="review-no-diff review-no-diff-success">✓ Le client n’a modifié aucun texte du référentiel.</p>';
    const adminDiffHtml=hasReviewSnapshot&&adminDifferences.length?comparisonTable('Corrections apportées par Me&YouToo','Ces éléments diffèrent maintenant de la version transmise par le client.',adminDifferences,'Version transmise par le client','Version Me&YouToo','review-meayt-version'):'';
