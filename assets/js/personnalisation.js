(()=>{
  const p=new URLSearchParams(location.search),theme=p.get('theme')||'sexisme',projectId=p.get('projectId')||'';let active=Math.max(0,Number(p.get('chapter')||0)),chapters=[],project=null;
  ...
  async function incompleteModal(index,title){
    ...
    const msg=st.rules.min!=null&&st.count<st.rules.min?`${st.rules.min} situations minimum sont nécessaires. Cette partie en contient ${st.count}.`:`Cette partie dépasse le maximum de ${st.rules.max} situations.`;
    const go=await window.StudioModal.confirm({eyebrow:'Composition du diagnostic',title:title||'Partie incomplète',message:`${ch.title} : ${msg}`,type:'info',cancelLabel:'Rester ici',confirmLabel:'Compléter cette partie'});
    if(go)location.href='composer.html'+q(index);
  }
  function card(ch,pr,pi){return `<article class="profile-excel-card"><div class="profile-excel-head"><div><small>Profil ${pi+1}/3</small><h3>${esc(pr.title)}</h3></div><span class="profile-color" style="background:${esc(pr.color)}"></span></div><div class="field"><label>Titre du profil</label><input data-profile-title="${pr.id}" value="${esc(pr.title)}"></div><div class="field"><label>Résumé du profil</label><textarea rows="4" data-profile-summary="${pr.id}">${esc(pr.summary)}</textarea></div><div class="field"><label>Contenu détaillé du profil</label><textarea rows="10" data-profile-content="${pr.id}">${esc(pr.content)}</textarea></div><div class="profile-method"><strong>Seuil :</strong> ${pr.scoring_min} → ${pr.scoring_max} · non modifiable</div></article>`;}
  function renderNav(){
    $('profiles-catalog-summary').textContent=`${chapters.length} parties · Questions et profils réunis dans un même parcours`;
    $('profiles-chapter-nav').innerHTML=chapters.map((ch,i)=>{
      const st=status(ch),blocking=firstInvalidIndex(i),blocked=blocking!==-1;
      return `<article class="creation-chapter-item ${i===active?'is-active':''} ${!st.valid?'is-incomplete':''}"><div class="creation-chapter-head"><span><small>Partie ${i+1}</small>${esc(ch.title)}</span><strong>${!st.valid&&st.rules.min!=null?`${st.count}/${st.rules.min} situations`:`${(ch.profiles||[]).length}/3`}</strong></div><div class="creation-chapter-tabs"><a href="composer.html${q(i)}">Questions</a><button class="${i===active?'is-active':''} ${blocked?'is-disabled':''}" data-profile-chapter="${i}" data-blocking-chapter="${blocking}" aria-disabled="${blocked}" type="button">Profils</button></div></article>`;
    }).join('');
  ...
  function render(){
    ...
    $('profile-chapter-kicker').textContent=`Partie ${active+1} · Profils`;$('profile-chapter-title').textContent=ch.title;$('profile-chapter-desc').textContent=active===chapters.length-1?'Le scoring de cette dernière partie est volontairement plus strict lorsque les situations comportent des enjeux liés à la loi.':'Personnalisez les trois profils issus du référentiel Me&YouToo.';$('profiles-root').innerHTML=`<div class="profile-excel-grid">${profiles.map((pr,pi)=>card(ch,pr,pi)).join('')}</div>`;$('profile-sticky-label').textContent=`Partie ${active+1}/${chapters.length} · ${ch.title}`;$('back-link').href='composer.html'+q(active);$('questions-step').href='composer.html'+q(active);
    const next=$('next-link');
    if(active<chapters.length-1){
      const blocking=firstInvalidIndex(active);next.href='composer.html'+q(active+1);next.textContent=`Questions de la partie ${active+2} →`;next.classList.toggle('is-disabled',blocking!==-1);next.setAttribute('aria-disabled',String(blocking!==-1));next.onclick=blocking!==-1?async e=>{e.preventDefault();await incompleteModal(blocking,'Complétez cette partie avant de poursuivre');}:null;
    }else{
  ...
  async function load(){try{if(!projectId){const created=await window.StudioProject.createOrResume(theme);location.replace(`personnalisation.html?theme=${encodeURIComponent(theme)}&projectId=${encodeURIComponent(created.id)}&chapter=0`);return;}const d=await api(`/api/projects/${projectId}/composer`);project=d.project;chapters=d.chapters||[];active=Math.min(active,Math.max(0,chapters.length-1));if(chapters.some(ch=>(ch.profiles||[]).length!==3))throw new Error('Le référentiel doit contenir exactement 3 profils par partie.');$('theme-name').textContent=project?.theme_title||'Autodiagnostic';$('profiles-catalog-title').textContent=project?.theme_title||'Autodiagnostic';await api(`/api/projects/${projectId}/progress`,{method:'PATCH',body:JSON.stringify({currentStep:'personnalisation'})});render();}catch(e){$('profiles-alert').hidden=false;$('profiles-alert').textContent=e.message;}}
