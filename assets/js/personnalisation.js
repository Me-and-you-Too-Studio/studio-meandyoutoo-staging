(()=>{
  ...
  async function load(){
    try{
      ...
      document.getElementById('profiles-root').innerHTML=chapters.map((ch,ci)=>{const profiles=[...(ch.profiles||[])].sort((a,b)=>colorRank(a.color)-colorRank(b.color)),legalNote=ci===chapters.length-1?'<p class="section-desc">Pour ce dernier chapitre, le scoring est volontairement plus strict, car les situations abordées comportent des enjeux liés à la loi.</p>':'<p class="section-desc">3 profils issus du référentiel Me&YouToo.</p>';return `<section class="profile-chapter"><div class="section-head"><div><h2 class="section-title">Chapitre ${ci+1} — ${esc(ch.title)}</h2>${legalNote}</div></div><div class="profile-excel-grid">${profiles.map((pr,pi)=>card(ch,ci,pr,pi)).join('')}</div></section>`;}).join('');
