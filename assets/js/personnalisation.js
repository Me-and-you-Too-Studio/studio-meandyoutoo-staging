(()=>{
  ...
  const countryNames={AR:'Argentine',AT:'Autriche',BR:'Brésil',CA:'Canada',CH:'Suisse',CL:'Chili',CN:'Chine',DE:'Allemagne',DK:'Danemark',ES:'Espagne',FR:'France',HK:'Hong Kong',JP:'Japon',KR:'Corée du Sud',MX:'Mexique',NO:'Norvège',PA:'Panama',PT:'Portugal',SE:'Suède',TW:'Taïwan',US:'États-Unis',UY:'Uruguay','250':'France','276':'Allemagne','032':'Argentine','040':'Autriche','076':'Brésil','124':'Canada','152':'Chili','156':'Chine','158':'Taïwan','208':'Danemark','724':'Espagne','840':'États-Unis','344':'Hong Kong','392':'Japon','484':'Mexique','578':'Norvège','591':'Panama','620':'Portugal','752':'Suède','756':'Suisse','858':'Uruguay','410':'Corée du Sud'};
  ...
  function projectCountries(){return [...new Set((Array.isArray(project?.country_codes)?project.country_codes:[]).concat(Array.isArray(project?.countries)?project.countries:[]).concat(project?.selected_country_code?[project.selected_country_code]:[]).map(x=>String(x||'').trim().toUpperCase()).filter(Boolean))];}
  ...
  function renderProfilesContext(){const root=$('profiles-context');if(!root)return;const countries=projectCountries(),locales=projectLocales(),multiCountry=countries.length>1,multiLocale=locales.length>1;if(!multiCountry&&!multiLocale){root.hidden=true;root.innerHTML='';return;}const parts=[];if(multiCountry)parts.push(`<div><span>Périmètres composés</span><div class="theme-availability-pills">${countries.map(c=>`<span class="theme-availability-pill is-scope">${esc(countryLabel(c))}</span>`).join('')}</div></div>`);if(multiLocale)parts.push(`<div><span>Langues actives</span><div class="theme-availability-pills">${locales.map(l=>`<span class="theme-availability-pill is-language">${esc(localeLabel(l))}</span>`).join('')}</div></div>`);const title=multiCountry&&multiLocale?'Périmètres et langues de la campagne':multiCountry?'Périmètres de la campagne':'Langues de la campagne';const note=multiLocale?'Les profils sont rédigés une seule fois puis leurs versions linguistiques peuvent être vérifiées séparément.':'Les mêmes profils sont utilisés pour tous les périmètres de cette campagne.';root.innerHTML=`<div class="profiles-context-head"><div><strong>${title}</strong>${infoDot(note)}</div></div><div class="profiles-context-grid ${parts.length===1?'is-single':''}">${parts.join('')}</div><p class="profiles-context-note">${note}</p>`;root.hidden=false;}
  ...
  const rules=ch=>isLegacyClientCampaign()?{min:null,max:null}:isStereotypes(ch)?{min:null,max:null}:{min:isAggression(ch)||isHostile(ch)?4:5,max:8};
  ...
  function card(ch,pr,pi){
    const ro=isReadOnly(),multiLocale=projectLocales().length>1,multiCountry=projectCountries().length>1;
    ...
    const profileHeadInfo=multiCountry?infoDot('Ce profil est utilisé pour tous les périmètres de la campagne.'):'';
    const titleInfo=multiLocale?infoDot('Titre du profil dans la langue de référence. Les autres langues se vérifient dans les versions linguistiques.'):infoDot('Titre du profil présenté au répondant dans sa restitution.');
    const summaryInfo=multiLocale?infoDot('Résumé du profil dans la langue de référence. Les autres langues se vérifient dans les versions linguistiques.'):infoDot('Résumé court présenté au répondant avec son profil.');
    const contentInfo=multiLocale?infoDot('Description détaillée du profil dans la langue de référence. Les autres langues se vérifient séparément.'):infoDot('Texte présenté au répondant pour expliquer son profil à la fin de l’autodiagnostic.');
    const translationRow=multiLocale?`<div class="composer-translation-row"><button class="button button-ghost composer-translation-button" type="button" data-profile-translations="${pr.id}" hidden>🌐 Vérifier les versions linguistiques</button>${infoDot('Ouvrez les versions linguistiques du profil : titre, résumé et description détaillée.')}</div>`:'';
    return `<article class="profile-excel-card" ${ro?'style="background:var(--royal-blue-tint);border-color:var(--line)"':''}><div class="profile-excel-head"><div><small>Profil ${pi+1}/3 ${profileHeadInfo}</small><h3>${esc(pr.title)}</h3></div><span class="profile-color" style="background:${esc(pr.color)}"></span></div><div class="field"><label>Titre du profil ${titleInfo}</label><input data-profile-title="${pr.id}" data-original-profile="${esc(originalTitle)}" value="${esc(pr.title)}" ${ro?'readonly aria-readonly="true" tabindex="-1" style="background:var(--surface-soft);pointer-events:none"':''}><div data-live-profile-diff="title:${esc(pr.id)}">${profileDiffBlock(originalTitle,pr.title)}</div></div><div class="field"><label>Résumé du profil ${summaryInfo}</label><textarea rows="4" data-profile-summary="${pr.id}" data-original-profile="${esc(originalSummary)}" ${ro?'readonly aria-readonly="true" tabindex="-1" style="background:var(--surface-soft);pointer-events:none"':''}>${esc(pr.summary)}</textarea><div data-live-profile-diff="summary:${esc(pr.id)}">${profileDiffBlock(originalSummary,pr.summary)}</div></div><div class="field"><label>Contenu détaillé du profil ${contentInfo}</label><textarea rows="10" data-profile-content="${pr.id}" data-original-profile="${esc(originalContent)}" ${ro?'readonly aria-readonly="true" tabindex="-1" style="background:var(--surface-soft);pointer-events:none"':''}>${esc(pr.content)}</textarea><div data-live-profile-diff="content:${esc(pr.id)}">${profileDiffBlock(originalContent,pr.content)}</div></div><div class="profile-method"><div class="profile-method-main"><span class="profile-method-label">Seuil de restitution</span><strong class="profile-method-range">${pr.scoring_min} → ${pr.scoring_max}</strong><span class="profile-method-lock">Non modifiable</span></div>${infoDot('Les seuils de restitution sont définis par Me&YouToo et ne sont pas modifiables.')}</div>${translationRow}</article>`;
  }
  ...
  async function openProfileTranslationModal(id){
    ...
    const legacyImported=isLegacyClientCampaign();
    const overlay=document.createElement('div');overlay.className='translation-overlay';overlay.innerHTML=`<section class="translation-modal" role="dialog" aria-modal="true"><header class="translation-head"><div><small>${legacyImported?'VERSIONS LINGUISTIQUES IMPORTÉES DU PROFIL':'TRADUCTION ET ADAPTATION LOCALE ÉVENTUELLE DU PROFIL'}</small><h2>${legacyImported?'Comparer les versions linguistiques importées':'Comparer avec la langue de référence'}</h2></div><button class="translation-close" type="button" aria-label="Fermer">×</button></header><div class="translation-toolbar"><strong>${legacyImported?'Langue à consulter':'Langue / adaptation à vérifier'}</strong><select data-translation-locale>${targets.map(l=>`<option value="${esc(l)}">${esc(localeLabel(l))}</option>`).join('')}</select></div><div class="profile-modal-context">${legacyImported?'<strong>Versions historiques importées.</strong> Le français et les autres langues proviennent directement de la campagne client existante.':'<strong>Profil commun à tous les périmètres.</strong> Vous vérifiez ici uniquement sa version linguistique : titre, résumé et description détaillée.'}</div><div class="translation-grid"><section class="translation-pane reference" data-reference-pane></section><section class="translation-pane" data-target-pane></section></div><footer class="translation-foot"><button class="button button-ghost" type="button" data-cancel>${isReadOnly()?'Fermer':'Annuler'}</button>${isReadOnly()?'':'<button class="button button-primary" type="button" data-save>Enregistrer et fermer</button>'}</footer></section>`;document.body.appendChild(overlay);
    ...
    function draw(){const loc=select.value,t=ctx.translations?.[loc]||{};const missing=!t.title||!t.summary||!t.content;const sourceCustomized=Boolean(ctx.sourceCustomized&&!legacyImported);referencePane.innerHTML=`<h3>${esc(localeLabel(referenceLocale))}${legacyImported?' · version importée':' · référence'}</h3><div class="translation-profile-block"><strong>Titre</strong><div class="translation-reference-text">${esc(reference.title||'')}</div><strong>Résumé</strong><div class="translation-reference-text">${esc(reference.summary||'')}</div><strong>Description détaillée</strong><div class="translation-reference-text">${esc(reference.content||'')}</div></div>`;target.innerHTML=`<h3>${esc(localeLabel(loc))}<span class="translation-status ${missing?'is-incomplete':''}">${legacyImported?(missing?'Version importée incomplète':'Version importée'):sourceCustomized?'À vérifier':missing?'Traduction incomplète':t.status==='client_version'?'Personnalisée':'Traduction Me&YouToo'}</span></h3>${sourceCustomized?`<div class="translation-sync-warning">⚠️ Le profil dans la langue de référence ${esc(localeLabel(referenceLocale))} a été modifié. Vérifiez les trois champs de cette version locale.</div>`:''}<div class="translation-local-note">${legacyImported?'Cette version provient directement de la campagne historique importée.':'Modifier cette version n’actualise pas automatiquement les autres langues.'}</div><label class="translation-profile-field"><strong>Titre</strong><input data-profile-target-title value="${esc(t.title||'')}" placeholder="Titre à compléter" ${isReadOnly()?'readonly':''}></label><label class="translation-profile-field"><strong>Résumé</strong><textarea rows="4" data-profile-target-summary placeholder="Résumé à compléter" ${isReadOnly()?'readonly':''}>${esc(t.summary||'')}</textarea></label><label class="translation-profile-field"><strong>Description détaillée</strong><textarea rows="8" data-profile-target-content placeholder="Description à compléter" ${isReadOnly()?'readonly':''}>${esc(t.content||'')}</textarea></label>`;}
  ...
  function projectMediaBlock(ch){
    ...
    const campaignLocales=projectLocales();
    const localeOptions=campaignLocales.length
      ? `<option value="">Toutes les langues actives</option>${campaignLocales.map(l=>`<option value="${esc(l)}">${esc(localeLabel(l))}</option>`).join('')}`
      : '<option value="">Langue non précisée</option>';
    return `<section class="profile-media-panel"><div class="profile-media-panel-head"><div><span class="profile-media-kicker">VIDÉOS DE RESTITUTION</span><h3>Vidéos affichées dans les profils</h3><p>Seules les vidéos compatibles avec les langues actives de cette campagne sont affichées. Pour un ancien autodiagnostic particulier, vous pouvez ajouter ici une vidéo spécifique sans modifier son contenu historique.</p></div>${infoDot('La Médiathèque est la source centrale. Une vidéo du catalogue n’est héritée par une campagne que si sa langue correspond à une langue active de cette campagne. Utilisez une association spécifique uniquement pour une exception legacy.')}</div><div class="profile-media-list">${profileMedia.length?profileMedia.map(item).join(''):'<p class="profile-media-empty">Aucune vidéo de restitution compatible avec cette campagne sur ce chapitre.</p>'}</div><div class="profile-media-add"><label><span>Ajouter une vidéo spécifique</span><select data-project-media-video><option value="">Choisir dans la Médiathèque…</option>${options}</select></label><label><span>Langue</span><select data-project-media-locale>${localeOptions}</select></label><label><span>Afficher sur</span><select data-project-media-profile><option value="">Les 3 profils</option><option value="0">Profil 1</option><option value="1">Profil 2</option><option value="2">Profil 3</option></select></label><button class="button button-primary" type="button" data-add-project-media ${options?'':'disabled'}>Associer la vidéo</button></div>${inherited.length?'<p class="profile-media-note">ⓘ « Héritée » signifie : association catalogue compatible avec les langues de cette campagne.</p>':''}</section>`;
  }
  ...
  function render(){
    ...
    const multiCountry=projectCountries().length>1,multiLocale=projectLocales().length>1;
    const pageLead=document.querySelector('.topbar .lead'),objectiveCopy=document.querySelector('.creation-objective p:not(.profile-autosave-note)');
    if(pageLead)pageLead.textContent=multiLocale?'Personnalisez les trois profils de chaque chapitre puis vérifiez leurs versions linguistiques.':multiCountry?'Personnalisez les trois profils communs aux différents périmètres de cette campagne.':'Personnalisez les trois profils de chaque chapitre qui seront restitués aux répondants.';
