(()=>{
  ...
  function bind(){
    if(isReadOnly())return;
    document.querySelectorAll('[data-profile-title],[data-profile-summary],[data-profile-content]').forEach(el=>el.addEventListener('change',async()=>{const id=el.dataset.profileTitle||el.dataset.profileSummary||el.dataset.profileContent,key=el.dataset.profileTitle?'title':el.dataset.profileSummary?'summary':'content';try{await api(`/api/projects/${projectId}/profiles/${id}`,{method:'PATCH',body:JSON.stringify({[key]:el.value})});translationContexts.delete(String(id));const btn=document.querySelector(`[data-profile-translations="${CSS.escape(String(id))}"]`);if(btn&&!btn.hidden){btn.insertAdjacentHTML('afterend','<span class="translation-inline-warning">⚠ Traductions et adaptations locales à vérifier après modification de la langue de référence</span>');}}catch(e){await window.StudioModal.alert({eyebrow:'Enregistrement du profil',title:'Modification non enregistrée',message:e.message,type:'error',confirmLabel:'J’ai compris'});}}));
